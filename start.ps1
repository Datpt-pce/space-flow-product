...
if (-not (Test-Path $envPath)) {
    ...
    Warn "Dien them CREDENTIALS_ENCRYPTION_KEY (64 ky tu hex); xem TEAM-SETUP.md."
...
$credentialKeyMatch = [regex]::Match($envContent, '(?m)^\s*CREDENTIALS_ENCRYPTION_KEY\s*=\s*["'']?([a-fA-F0-9]{64})["'']?\s*(?:#.*)?$')
if (-not $credentialKeyMatch.Success) {
    Fail "Can CREDENTIALS_ENCRYPTION_KEY hop le (64 ky tu hex) trong .env truoc khi chay Docker."
    Warn "May da co du lieu: khoi phuc key cu; KHONG tao key moi de thay key da mat."
    Warn "May moi: xem TEAM-SETUP.md de tao key va luu ban sao rieng."
    exit 1
}
