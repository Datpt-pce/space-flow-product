...
function attachAgentServer(httpServer) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type === 'resource-sample') {
        if (!agentId || onlineAgents.get(userId) !== ws || raw.length > 8192 || Date.now() - lastResourceAt < 10000) return;
        if (!db.prepare("SELECT 1 FROM users WHERE id=? AND status='active'").get(userId)) return;
        lastResourceAt = Date.now();
        try { require('../analytics/runtime').saveResource('agent', userId, msg.sample); } catch { /* telemetry cannot break relay */ }
        return;
      }
