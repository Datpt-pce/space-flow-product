...
async function run(workflow, uploadsDir, send, startNodeId = null, resume = false, userId = null, runContext = {}) {
  ...
  topoSort(activeNodes, activeEdges); // Validate before reserving busy IDs so malformed graphs cannot leak locks.
  const busyKey = id => `${userId || 'local'}:${id}`;
  const conflict = activeNodes.find(n => busyNodeIds.has(busyKey(n.id)));
  if (conflict) {
    throw new Error(`Node "${conflict.id}" đang được một tiến trình khác chạy, đợi nó xong rồi thử lại.`);
  }
  for (const n of activeNodes) busyNodeIds.add(busyKey(n.id));
  ...
  async function runNode(nodeId) {
    ...
    const policy = require('./effectPolicy').retryPolicy(node);
    const maxTries = policy.maxAttempts;
    const retryDelay = policy.delayMs;
    ...
    for (let attempt = 1; attempt <= maxTries; attempt++) {
      ...
      let analyticStart = null;
      const attemptEvent = (state) => {
        try { send('analyticsAttempt', { id: analyticId, nodeId, nodeType: node.type, attempt,
          location: runContext.executionLocation || 'server', state,
          ...(analyticStart !== null && state !== 'started' ? { durationMs: performance.now() - analyticStart } : {}) }); }
        catch { console.warn('[analytics] attempt observation unavailable'); }
      };
      try {
        runContext.signal?.throwIfAborted();
        send('nodeStart', { nodeId, attempt, effectClass: require('./effectPolicy').effectClass(node.type) });
        analyticStart = performance.now();
        attemptEvent('started');
        output = await require('./executionContext').run(context, () => invokeExecutor(node, manifest, installPath, inputs, config, context));
        runContext.signal?.throwIfAborted();
        ...
        attemptEvent('succeeded');
        lastErr = null;
        break;
      } catch (err) {
        if (analyticStart !== null) attemptEvent(runContext.signal?.aborted ? 'cancelled' : 'failed');
        if (runContext.signal?.aborted) throw err;
        lastErr = err;
        if (attempt < maxTries) {
          send('log', { nodeId, message: `Retry ${attempt}/${maxTries - 1} after error: ${err.message}`, level: 'warn' });
          await sleep(Math.min(30000, retryDelay * 2 ** (attempt - 1)) * (0.5 + Math.random() * 0.5));
        }
      }
    }
  ...
  function dispatch() {
    if (aborted) return;
    if (runContext.signal?.aborted) { aborted = true; abortError = new Error('Run cancelled'); return; }
    for (const nodeId of ready) {
      if (runningCount >= 8) break;
  ...
  try {
    ...
    for (const n of activeNodes) busyNodeIds.delete(busyKey(n.id));
