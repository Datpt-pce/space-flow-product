...
function spawnPython(scriptPath, payload, onLine) {
  return new Promise((resolve, reject) => {
    const signal = require('./executionContext').getStore()?.signal;
    if (signal?.aborted) return reject(signal.reason || new Error('Run cancelled'));
    // Kill the process group/tree so FFmpeg children cannot outlive STOP.
    const proc = spawn(resolvePythonExe(), [scriptPath], { windowsHide: true, detached: process.platform !== 'win32' });
    const abort = () => {
    ...
    proc.stdin.end();
    proc.stdin.on('error', () => {}); // A concurrent STOP can close stdin before the payload flushes.
