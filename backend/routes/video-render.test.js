...
async function main() {
  ...
  try {
    ...
    await check('cancel mid-flight: DB status becomes cancelled AND the real ffmpeg.exe process actually dies', async () => {
      ...
      // Running includes preparation before spawn. Linux has no tasklist guard above, so
      // wait for the actual worker handle and cancel it instead of racing a no-op.
      await waitFor(() => cancelRenderJob(jobId), { intervalMs: 10 });
    ...
    await check('verifyRenderOutput: rejects a truncated/corrupt file (real ffprobe), accepts a real render', async () => {
      const badPath = path.join(require('../utils/dataPaths').uploads, `verify-test-corrupt-${crypto.randomUUID()}.mp4`);
