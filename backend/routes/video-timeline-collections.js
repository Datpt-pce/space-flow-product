...
function permanentlyDeleteCollection(collectionId) {
  ...
  if (db.prepare('SELECT id FROM video_batch_projects WHERE collection_id = ?').get(collectionId)) throw new Error('Collection thuộc Batch Creative Lab; khôi phục qua Lab để giữ draft và lịch sử.');
