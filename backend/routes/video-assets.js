...
const mediaUpload = require('multer')({ storage: require('multer').memoryStorage(), limits: { fileSize: 256 * 1024 * 1024, files: 1, fields: 8, parts: 9, fieldSize: 8192, fieldNestingDepth: 2 } });
const UPLOADS_DIR = require('../utils/dataPaths').uploads;
const previewQueue = require('../video/assetPreviewQueue').createAssetPreviewQueue(db, UPLOADS_DIR);
...
function toPublicAsset(row) {
  return {
    ...
    originalSourcePath: row.original_source_path || null,
    ...
    previewStatus: row.preview_status || (row.proxy_path ? 'done' : null),
    previewProgress: row.preview_progress ?? (row.proxy_path ? 100 : 0),
    previewError: row.preview_error || null,
    status: row.status,
...
function makeRunJob(userId, needsAgent) {
  return async function runJob(kind, payload, onProgress = () => {}) {
    if (!needsAgent) return runVideoJob(kind, payload, onProgress);
...
//
// A synchronous job failure does NOT throw — it's recorded as status:'error' on
...
// (unrecognized extension, preflight) throws to the caller. Public imports defer
// thumbnail/proxy work: preview errors are separate and leave the original usable.
const importsByIdentity = new Map();
async function importAsset(ownerId, sourcePath, runJob, { skipPreflight = false, forceKind = null, sourceLocality = 'agent', deferPreview = false } = {}) {
  const replacing = db.prepare("SELECT id FROM video_batch_deliveries WHERE (owner_id=? OR ?='agent') AND state IN ('confirmed','prepared') AND json_extract(intent_json,'$.mode')='replace' AND json_extract(intent_json,'$.anchorPath')=? LIMIT 1").get(ownerId,process.env.SPACE_FLOW_MODE || 'agent',sourcePath);
  if(replacing) throw Object.assign(new Error('Nguồn đang được thay file. Hoàn tất delivery trước khi import lại.'),{status:409});
  const localManagedPath = require('../services/artifacts').withinUploads(sourcePath);
  const managed = localManagedPath ? db.prepare("SELECT owner_id FROM artifacts WHERE path=? AND state='active'").get(localManagedPath) : null;
  if (managed && managed.owner_id !== ownerId) throw Object.assign(new Error('Artifact access denied'), { status: 403 });
  ...
  try {
    ...
    const { contentHash, sizeBytes, machineId } = await runJob('hash', { path: sourcePath });
    if(db.prepare("SELECT id FROM video_batch_deliveries WHERE owner_id=? AND state IN ('confirmed','prepared') AND json_extract(intent_json,'$.mode')='replace' AND json_extract(intent_json,'$.expectedHash')=? LIMIT 1").get(ownerId,contentHash)) throw new Error('Nguồn này đang được thay file. Hoàn tất delivery trước khi import lại.');
    db.prepare('UPDATE video_assets SET source_machine_id=? WHERE id=?').run(machineId || null,id);
    identity = JSON.stringify([ownerId, sourcePath, contentHash, kind, sourceLocality]);
    if (importsByIdentity.has(identity)) {
      db.prepare('DELETE FROM video_assets WHERE id = ?').run(id);
      const shared = await importsByIdentity.get(identity);
      // A synchronous internal caller may join a public metadata-only import.
      // Re-enter after that import settles to await/repair its pending preview.
      if (!deferPreview && shared?.kind === 'video' && !shared.proxy_path) {
        return importAsset(ownerId, sourcePath, runJob, { skipPreflight: true, forceKind, sourceLocality });
      }
      return shared;
    }
    importsByIdentity.set(identity, new Promise(resolve => { finishImport = resolve; }));
    const existing = db.prepare("SELECT * FROM video_assets WHERE owner_id = ? AND source_path = ? AND content_hash = ? AND kind = ? AND source_locality = ? AND status = 'ok' ORDER BY created_at, id LIMIT 1").get(ownerId, sourcePath, contentHash, kind, sourceLocality);
    if (existing) {
      db.prepare('DELETE FROM video_assets WHERE id = ?').run(id);
      db.prepare("UPDATE video_assets SET source_path = ?, source_machine_id=COALESCE(?,source_machine_id), removed_from_bin_at = NULL, last_seen_at = datetime('now') WHERE id = ?").run(sourcePath, machineId || null, existing.id);
      id = existing.id;
      if (kind === 'video' && deferPreview && ['queued', 'running'].includes(existing.preview_status)) {
        previewQueue.enqueue(existing, runJob);
        return db.prepare('SELECT * FROM video_assets WHERE id = ?').get(id);
      }
      if (!deferPreview && previewQueue.pending(id)) {
        await previewQueue.pending(id);
        Object.assign(existing, db.prepare('SELECT * FROM video_assets WHERE id = ?').get(id));
      }
    ...
    if (kind === 'video') {
      if (deferPreview) {
        db.prepare("UPDATE video_assets SET preview_status='queued', preview_progress=0, preview_error=NULL WHERE id=?").run(id);
        previewQueue.enqueue(db.prepare('SELECT * FROM video_assets WHERE id=?').get(id), runJob);
      }
      else db.prepare("UPDATE video_assets SET preview_status='done', preview_progress=100, preview_error=NULL WHERE id=?").run(id);
    }
  } catch (err) {
...
router.get('/', async (req, res) => {
  try {
    ...
    await refreshAssetAvailability(rows, req.user.id);
    resumePreviews(rows, req.user.id);
    res.json(rows.map(toPublicAsset));
  } catch (err) { res.status(500).json({ error: err.message }); }
});

function resumePreviews(rows, ownerId) {
  for (const row of rows) {
    if (!['queued', 'running'].includes(row.preview_status) || row.status !== 'ok') continue;
    const remote = (process.env.SPACE_FLOW_MODE || 'agent') === 'server' && row.source_locality !== 'server';
    if (remote && !require('../ws/agentServer').isAgentOnline(ownerId)) continue;
    previewQueue.enqueue(row, makeRunJob(ownerId, remote));
  }
}

router.post('/:id/preview/retry', (req, res) => {
  ...
  if (row.kind !== 'video' || row.status !== 'ok') return res.status(409).json({ error: 'Cần nối lại video gốc trước khi tạo preview.' });
  if (row.preview_status !== 'error') return res.json(toPublicAsset(row));
  const remote = (process.env.SPACE_FLOW_MODE || 'agent') === 'server' && row.source_locality !== 'server';
  const runJob = remote ? resolveRunJob(req, res) : makeRunJob(req.user.id, false);
  if (!runJob) return;
  previewQueue.enqueue(row, runJob);
  res.json(toPublicAsset(db.prepare('SELECT * FROM video_assets WHERE id=?').get(row.id)));
});

async function refreshAssetAvailability(rows, ownerId) {
  const serverMode = (process.env.SPACE_FLOW_MODE || 'agent') === 'server';
  for (const row of rows) {
    const remote = serverMode && row.source_locality !== 'server';
    if (!remote && row.status === 'ok') {
      try { fs.statSync(toContainerPath(row.source_path)); }
      catch (err) {
        // A transient permission/I/O failure is not proof that the source disappeared.
        if (!['ENOENT', 'ENOTDIR'].includes(err.code)) continue;
        db.prepare("UPDATE video_assets SET status = 'offline' WHERE id = ? AND status = 'ok' AND source_path = ?").run(row.id, row.source_path);
        row.status = 'offline';
      }
    }
    if (row.status !== 'offline' || !row.content_hash) continue;
    if (remote && !require('../ws/agentServer').isAgentOnline(ownerId)) continue;
    try {
      // Same-path recovery must verify bytes, just like explicit Relink. Checking
      // existence alone could silently attach a different file to saved clips/pins.
      const { contentHash } = await makeRunJob(ownerId, remote)('hash', { path: row.source_path });
      if (contentHash !== row.content_hash) continue;
      const result = db.prepare("UPDATE video_assets SET status = 'ok', error_message = NULL, last_seen_at = datetime('now') WHERE id = ? AND status = 'offline' AND source_path = ? AND content_hash = ?")
        .run(row.id, row.source_path, row.content_hash);
      if (result.changes) { row.status = 'ok'; row.error_message = null; }
    } catch { /* Missing file or disconnected agent: keep the existing link for recovery. */ }
  }
}
...
router.post('/import', async (req, res) => {
  ...
  try {
    const row = await importAsset(req.user.id, sourcePath, runJob, { deferPreview: true });
...
router.post('/upload', (req, res) => {
  mediaUpload.single('file')(req, res, async error => {
    ...
    const runJob = resolveRunJob(req, res); if (!runJob) return;
    try {
      const originalName = typeof req.body.originalName === 'string' && req.body.originalName.length <= 255 && detectKind(req.body.originalName) === detectKind(req.file.originalname)
        ? req.body.originalName : req.file.originalname;
      const { path: savedPath } = await runJob('save-blob', { dataBase64: req.file.buffer.toString('base64'), extension: path.extname(req.file.originalname).slice(1).toLowerCase(), originalName });
      res.json(toPublicAsset(await importAsset(req.user.id, savedPath, runJob, { deferPreview: true })));
    } catch (err) { res.status(400).json({ error: err.message }); }
  });
});
...
async function relinkAsset(assetId, ownerId, newPath, runJob) {
  ...
  let newHash, machineId;
  try {
    ({ contentHash: newHash, machineId } = await runJob('hash', { path: newPath }));
  } catch (err) {
  ...
  const replacement = db.prepare("SELECT id FROM video_batch_deliveries WHERE owner_id=? AND state<>'planned' AND json_extract(intent_json,'$.mode')='replace' AND (json_extract(intent_json,'$.anchorPath')=? OR state IN ('confirmed','prepared')) AND json_extract(intent_json,'$.expectedHash')=? LIMIT 1").get(ownerId,newPath,newHash);
  if(replacement) return {ok:false,status:409,error:'File này đang hoặc đã được thay bằng delivery. Relink bản gốc từ backup hoặc một đường dẫn khác.'};

  db.prepare("UPDATE video_assets SET source_path = ?, source_machine_id=COALESCE(?,source_machine_id), status = 'ok', error_message = NULL, last_seen_at = datetime('now') WHERE id = ?")
    .run(newPath, machineId || null, assetId);
...
async function deleteAsset(assetId, ownerId) {
  ...
  const labUse = require('../video/batchReferences').assetReference(db, ownerId, assetId);
  if (labUse) return { ok: false, status: 409, error: labUse };
...
module.exports.refreshAssetAvailability = refreshAssetAvailability;
