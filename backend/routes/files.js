...
router.post('/cleanup', (req, res, next) => {
  try { res.json(require('../services/artifacts').cleanup(req.user.id)); } catch (error) { next(error); }
});
...
const MEDIA_EXTS = ['.jpg', '.jpeg', '.png', '.webp', '.gif', '.bmp', '.tiff', '.mp4', '.mov', '.avi', '.mkv', '.webm', '.m4v', '.mp3', '.wav', '.aac', '.flac', '.m4a', '.ogg', '.opus', '.aiff', '.aif', '.wma'];
...
router.post('/open-folder', (req, res) => {
  ...
  exec(`explorer /select,"${absPath}"`, { windowsHide: true }, (err) => {
    if (err) exec(`explorer "${dir}"`, { windowsHide: true });
  });
  res.json({ ok: true });
});
...
router.get('/preview', (req, res) => {
  ...
  const artifactService = require('../services/artifacts');
  const relative = path.relative(UPLOADS_DIR, absPath);
  if ((!relative.startsWith('..') && !path.isAbsolute(relative)) && !artifactService.canRead(absPath, req.user.id)) {
...
router.post('/download-zip', (req, res) => {
  ...
  if (files.some(file => typeof file !== 'string' || !require('../services/artifacts').canRead(file, req.user.id))) {
    return res.status(403).json({ error: 'File access denied' });
  }
