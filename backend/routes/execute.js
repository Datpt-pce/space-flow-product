...
router.post('/', async (req, res) => {
  ...
  try {
    if (needsAgent) {
      // resize-upload-v2's config only stores a credential NAME — the agent has no access to
      // the central credentials/resize_link_catalogs tables, so resolve the real value HERE
      // (where req.user is the authenticated session) and stash it into node.config before the
      // whole workflow is relayed — see specs/resize-upload-v2-central-credentials.md.
      ...
      for (const node of workflow.nodes || []) {
        if (node.type === 'resize-upload-v2') {
          ...
          const cred = getCredential(node.config.asana_credential_name, req.user.id);
          node.config.__resolved_asana_pat = cred?.data?.token || '';
          node.config.__resolved_links = getLinkCatalog(req.user.id);
        }
      }
