...
async function runRenderJobAsync(jobId, projectId, ownerId) {
  ...
  try {
    ...
    require('../analytics/jobLocation').recordJobLocation(db, 'render', jobId, needsAgent ? 'agent' : 'server');
    const sourceHashes = {};
...
router.get('/:projectId/render/:jobId/download', (req, res) => {
  ...
  const batchOutput = db.prepare('SELECT 1 FROM video_batch_run_items WHERE timeline_id=?').get(row.project_id);
  const filename = batchOutput ? `${require('../video/batchOutputName').batchOutputName([project?.name])}.mp4`
    : `${name}-r${row.pinned_seq ?? 0}-${row.preset_id || 'original'}.mp4`;
  res.download(row.output_path, filename, (err) => {
    if (!err) { try { require('../analytics/outputUsage').recordOutputUsage(db, req.user.id, row.id); } catch { /* transfer succeeded; metrics are best effort */ } }
