...
// Resolve a workspace project through one of its timelines. Never trust a client
// supplied list of members for rename/archive or cross-owner scope.
function workspaceGroup(row, ownerId) {
  return row.collection_id ? db.prepare('SELECT id, name FROM video_timeline_collections WHERE id = ? AND owner_id = ? AND archived_at IS NULL').get(row.collection_id, ownerId) : null;
}

router.put('/:id/workspace-group', (req, res) => {
  const row = db.prepare('SELECT * FROM video_projects WHERE id = ?').get(req.params.id);
  if (!requireActiveOwner(req, res, row)) return;
  const name = req.body.name;
  if (typeof name !== 'string' || !name.trim() || name.trim().length > 200) return res.status(400).json({ error: 'Tên project cần từ 1 đến 200 ký tự.' });
  const group = workspaceGroup(row, req.user.id);
  if (group) db.prepare("UPDATE video_timeline_collections SET name = ?, updated_at = datetime('now') WHERE id = ?").run(name.trim(), group.id);
  else db.prepare("UPDATE video_projects SET name = ?, updated_at = datetime('now') WHERE id = ?").run(name.trim(), row.id);
  res.json({ name: name.trim() });
});

router.delete('/:id/workspace-group', (req, res) => {
  const row = db.prepare('SELECT * FROM video_projects WHERE id = ?').get(req.params.id);
  if (!requireActiveOwner(req, res, row)) return;
  ...
  try {
    ...
    const group = workspaceGroup(row, req.user.id);
    const members = group ? db.prepare('SELECT id FROM video_projects WHERE collection_id = ? AND owner_id = ? AND archived_at IS NULL').all(group.id, req.user.id) : [row];
    for (const member of members) archiveProject(member.id);
    db.exec('COMMIT'); transaction = false;
    res.json({ deletedTimelineIds: members.map(member => member.id) });
...
function permanentlyDeleteProject(projectId) {
  ...
  const labUse = require('../video/batchReferences').projectReference(db, projectId);
  if (labUse) throw new Error('Timeline đang được ghim trong Lab. Bỏ item khỏi list trước khi xoá vĩnh viễn.');
