...
async function main() {
  ...
  const secretPath = path.join(os.tmpdir(), `${path.basename(scratchDir)}-outside.txt`);
  fs.writeFileSync(secretPath, 'TOP-SECRET-SHOULD-NOT-BE-READABLE', { flag: 'wx' });
  ...
  const packageJsonPath = path.join(REPO_ROOT, 'backend', 'package.json');
