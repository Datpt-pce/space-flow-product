...
function bundleExecutor(executorPath) {
  // npm can install this CLI as native ELF on Linux and a JS launcher on Windows.
  // The supported JS API selects the correct executable without assuming either layout.
  return require('esbuild').buildSync({
    entryPoints: [executorPath], bundle: true, format: 'iife', globalName: GLOBAL_EXPORT_NAME,
    platform: 'node', alias: { path: 'path-browserify' }, logLevel: 'error', write: false,
