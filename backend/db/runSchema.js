var n=Object.defineProperty;var r=(e,E)=>n(e,"name",{value:E,configurable:!0});const crypto=require("crypto"),sql=`
...
`;function ensureRunSchema(e){const E=crypto.createHash("sha256").update(sql).digest("hex"),N=e.prepare("SELECT checksum FROM schema_migrations WHERE id = ?").get("002-durable-runs");if(N){if(N.checksum!==E)throw new Error("Migration checksum mismatch: 002-durable-runs");return}e.exec("BEGIN IMMEDIATE");try{e.exec(sql),e.prepare("PRAGMA table_info(workflows)").all().some(T=>T.name==="revision")||e.exec("ALTER TABLE workflows ADD COLUMN revision INTEGER NOT NULL DEFAULT 0"),e.prepare("INSERT INTO schema_migrations (id,checksum) VALUES (?,?)").run("002-durable-runs",E),e.exec("COMMIT")}catch(T){throw e.exec("ROLLBACK"),T}}r(ensureRunSchema,"ensureRunSchema"),module.exports={ensureRunSchema};
