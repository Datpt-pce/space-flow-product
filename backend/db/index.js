...
const DB_PATH = require('../utils/dataPaths').database;
const DB_DIR = path.dirname(DB_PATH);
fs.mkdirSync(DB_DIR, { recursive: true });
const finishSchemaUpgrade = require('./schemaBackup').prepare(DB_PATH);
...
if (process.env.SF_IMPORT_LEGACY !== '0' && credentialCount === 0 && fs.existsSync(LEGACY_STORE_PATH)) {
...
if (process.env.SF_IMPORT_LEGACY !== '0' && linkCatalogCount === 0 && fs.existsSync(LEGACY_LINKS_PATH)) {
...
require('./readinessSchema').ensureReadinessSchema(db);
require('./runSchema').ensureRunSchema(db);
require('./artifactReferencesSchema').ensureArtifactReferencesSchema(db);
require('../contributions/schema').ensureContributionSchema(db);
require('../analytics/schema').ensureAnalyticsSchema(db);
finishSchemaUpgrade();
require('../observability/database').attach(db);
module.exports = db;
