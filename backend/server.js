...
require('./utils/runtimeConfig').preflight();
const express = require('express');
const cors = require('cors');
const cookieParser = require('cookie-parser');
const path = require('path');
const fs = require('fs');
const db = require('./db'); // opens/creates the sqlite DB + runs schema on startup
...
const { resolveResizeUploadV3 } = require('./middleware/resolveResizeUploadV3');

const app = express();
if (process.env.SF_REVIEW_INSTANCE) app.use((req, res, next) => { res.setHeader('X-SF-Review-Instance', process.env.SF_REVIEW_INSTANCE); next(); });
if (process.env.SF_TRUSTED_PROXIES) app.set('trust proxy', process.env.SF_TRUSTED_PROXIES.split(',').map(value => value.trim()).filter(Boolean));
const telemetry = require('./observability/http').createTelemetry();
...
const WORKFLOWS_DIR = require('./utils/dataPaths').workflows;
...
app.use('/api/analytics/events', express.json({ limit: '32kb' }));
app.use(express.json({ limit: '50mb' }));
app.use(cookieParser());
const health = require('./observability/health').installHealth(app, db, telemetry, requireAuth, requireAdmin);
app.use('/uploads', requireAuth, require('./services/artifacts').serve);

app.use('/api/auth', require('./routes/auth'));
app.use('/api/analytics', requireAuth, requireCsrf, require('./routes/analytics').createAnalyticsRouter(db));
app.get('/api/admin-console/analytics', requireAuth, requireAdmin, (req, res, next) => {
  res.setHeader('Cache-Control', 'no-store');
  try { res.json(require('./analytics/query').queryAnalytics(db, req.query)); } catch (error) { next(error); }
});
...
app.use('/api/contributions', requireAuth, requireCsrf, require('./routes/contributions'));
...
app.use('/api/video-batch', requireAuth, requireCsrf, require('./routes/video-batch'));
app.use('/api/creative-assistant', requireAuth, requireCsrf, require('./routes/creative-assistant'));
...
app.use('/api/runs', requireAuth, requireCsrf, require('./routes/runs').router);
app.use('/api/upload', requireAuth, requireCsrf, require('./routes/upload'));
// Server-owned uploads are served here before local-files relay; user-agent files still use the relay.
app.get('/api/files/preview', requireAuth, (req, res, next) => {
  if (typeof req.query.path !== 'string') return next();
  const file = path.resolve(req.query.path);
  const relative = path.relative(UPLOADS_DIR, file);
  if (relative.startsWith('..') || path.isAbsolute(relative)) return next();
  if (!require('./services/artifacts').canRead(file, req.user.id)) return res.status(404).json({ error: 'Artifact not found' });
...
app.use('/api/resize-upload-v3', internalOrAuth, requireCsrf, resolveResizeUploadV3, relayToAgent({ only: ['/run', '/preview'] }), require('./routes/resize-upload-v3'));
...
app.use('/api/admin-console', requireAuth, requireAdmin, requireCsrf, require('./routes/admin-console'));
...
app.use((err, req, res, next) => {
  console.error(JSON.stringify({ timestamp: new Date().toISOString(), level: 'error', action: 'request.error', request_id: req.requestId, code: err.code || 'INTERNAL_ERROR' }));
...
httpServer.listen(PORT, process.env.SF_BIND_HOST || undefined, () => {
  require('./analytics/runtime').start();
  require('./services/workflowRunner').runs.start();
...
async function shutdown() {
  ...
  shuttingDown = true;
  health.drain();
  require('./analytics/runtime').stop();
  httpServer.close();
  require('./engine/scheduler').stop();
  require('./graph/reindexQueue').stop();
  require('./services/sheetSyncWorker').stop();
  await require('./contributions/runtime').worker.stop();
  const forced = setTimeout(() => process.exit(1), 25000); forced.unref();
  const drained = await require('./services/workflowRunner').runs.stop(20000);
  const renderActive = db.prepare("SELECT COUNT(*) AS n FROM video_render_jobs WHERE status='running'").get().n;
  console.log(JSON.stringify({ timestamp: new Date().toISOString(), level: 'info', action: 'server.shutdown', drained, render_active: renderActive }));
  httpServer.closeAllConnections();
  // Leave leases for startup recovery when a provider/render cannot finish inside the deadline.
  if (drained && !renderActive) db.close();
  process.exit(drained && !renderActive ? 0 : 1);
}
