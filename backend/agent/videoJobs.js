...
async function runVideoJob(kind, payload, onProgress, jobId) {
  ...
  switch (kind) {
    ...
    case 'capability-snapshot': {
      const uploads = require('../utils/dataPaths').uploads;
      fs.mkdirSync(uploads, { recursive: true });
      return buildCapabilitySnapshot(uploads);
    }
    ...
    case 'save-blob': {
      ...
      const dir = path.join(require('../utils/dataPaths').uploads, 'video-recordings', ...(payload.originalName ? [crypto.randomUUID()] : []));
    ...
    case 'hash':
      return { contentHash: await assetService.hashFile(sourcePath), sizeBytes: fs.statSync(sourcePath).size, machineId:require('./mediaDelivery').machineId() };

    case 'delivery-info':
    case 'delivery-prepare':
    case 'delivery-commit':
    case 'delivery-status':
      return require('./mediaDelivery').runDelivery(kind, payload);

    case 'source-begin':
    case 'source-chunk':
    case 'source-finish':
    case 'source-abort':
      return require('../video/sourceTransfer').createSourceReceiver(require('path').join(require('../utils/dataPaths').uploads, 'video-source-cache'))(kind, payload);
    ...
    case 'speech':
      return require('./speechJob').runSpeechJob({ ...payload, path:sourcePath, referencePath:toContainerPath(payload.referencePath) });
