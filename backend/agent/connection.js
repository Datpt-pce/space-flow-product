...
const CONFIG_PATH = path.join(require('../utils/dataPaths').config, 'agent.json');
...
function connect(port) {
  ...
  ws.on('message', async (raw) => {
    ...
    if (msg.type === 'registered') {
      stopAnalytics?.();
      stopAnalytics = require('../analytics/runtime').subscribe(sample => {
        if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'resource-sample', sample }));
      });
    ...
    if (msg.type === 'run') {
      ...
      if (typeof runId !== 'string' || runningWorkflows.has(runId)) return;
      const controller = new AbortController();
      runningWorkflows.set(runId, controller);
      const send = (event, data) => {
        if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'event', runId, event, data }));
      };
      try {
        require('../contracts/workflow').parseWorkflow(workflow);
        await executor.run(workflow, UPLOADS_DIR, send, startNodeId ?? null, !!resume, null, {
          signal: controller.signal, runId, requestId: msg.requestId, correlationId: msg.correlationId, executionLocation: 'agent',
        });
    ...
    if (msg.type === 'cancel-job') {
      runningWorkflows.get(msg.runId)?.abort();
