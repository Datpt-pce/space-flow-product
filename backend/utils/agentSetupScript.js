var i=Object.defineProperty;var o=(n,e)=>i(n,"name",{value:e,configurable:!0});function buildAgentSetupScript(n,e){const a=require("fs").readFileSync(require("path").join(__dirname,"../agent/assets/tray.ico")).toString("base64"),t=4010;return`# Space Flow Agent - cai dat tu dong
# Tao boi Settings -> Agent. Chay 1 lan de pairing may nay lam agent cho ${e}
...
$AgentConfigPath = "backend\\config\\agent.json"
$AgentConfig = if (Test-Path $AgentConfigPath) { Get-Content -Raw $AgentConfigPath | ConvertFrom-Json } else { [PSCustomObject]@{} }
$AgentConfig | Add-Member -NotePropertyName agentToken -NotePropertyValue $AgentToken -Force
$AgentConfig.PSObject.Properties.Remove('agentTokenProtected')
$AgentConfig | ConvertTo-Json -Depth 20 | Set-Content -Path $AgentConfigPath -Encoding utf8
if (Test-Path "backend\\agent\\releases\\tokenStore.js") {
  node -e "require('./backend/agent/releases/tokenStore').protect()"
  if ($LASTEXITCODE -ne 0) { throw "Khong bao ve duoc token Agent tren Windows." }
}
...
$ExistingEnvLines + @("SPACE_FLOW_MODE=agent", "CENTRAL_SERVER_URL=$CentralServerUrl", "PORT=${t}") | Set-Content -Path ".env" -Encoding ascii
...
function Get-AgentPid {
  $Conn = Get-NetTCPConnection -LocalPort ${t} -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
...
$IconBytes = [Convert]::FromBase64String('${a}')
...
# lan cai truoc) - no van giu port ${t} va khoa file DB WAL, khien process moi khoi dong xong
# chet ngay lap tuc (EADDRINUSE hoac SQLITE_BUSY khi mo DB o dong dau tien cua server.js).
Write-Host "Kiem tra + dung tien trinh agent cu (neu co) dang giu port ${t}..."
...
try {
  $OldConns = Get-NetTCPConnection -LocalPort ${t} -State Listen -ErrorAction SilentlyContinue
  foreach ($c in $OldConns) {
    Write-Host "Dung tien trinh cu (PID $($c.OwningProcess)) dang chiem port ${t}..."
...
$AgentListener = Get-NetTCPConnection -LocalPort ${t} -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
...
`}o(buildAgentSetupScript,"buildAgentSetupScript"),module.exports={buildAgentSetupScript};
