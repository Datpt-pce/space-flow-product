var d=Object.defineProperty;var n=(N,e)=>d(N,"name",{value:e,configurable:!0});const sql=`
...
`;function ensureAnalyticsSchema(N){N.exec(sql);const e="(SELECT value FROM analytics_settings WHERE key='enabled')='1'",s="CAST((julianday('now')-2440587.5)*86400000 AS INTEGER)";for(const[a,t,E,i,T]of[["flow_runs","workflow","state","NEW.created_at","NEW.updated_at"],["video_render_jobs","render","status","CAST(strftime('%s',NEW.created_at) AS INTEGER)*1000",s],["video_batch_deliveries","delivery","state","CAST(strftime('%s',NEW.created_at) AS INTEGER)*1000",s]]){if(!N.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(a))continue;const L=`NEW.${E} IN ('succeeded','failed','unknown','cancelled','done','error')`;N.exec(`CREATE TRIGGER IF NOT EXISTS analytics_${t}_insert AFTER INSERT ON ${a} WHEN ${e}
    BEGIN INSERT OR IGNORE INTO analytics_jobs(kind,id,owner_id,state,created_at,started_at,ended_at,updated_at)
      VALUES ('${t}',NEW.id,NEW.owner_id,NEW.${E},${i},CASE WHEN NEW.${E}='running' THEN ${T} END,
        CASE WHEN ${L} THEN ${T} END,${T}); END;
    CREATE TRIGGER IF NOT EXISTS analytics_${t}_state AFTER UPDATE OF ${E} ON ${a}
      WHEN OLD.${E} IS NOT NEW.${E} AND ${e}
    BEGIN UPDATE analytics_jobs SET state=NEW.${E}, updated_at=${T},
      started_at=CASE WHEN NEW.${E}='running' THEN COALESCE(started_at,${T}) ELSE started_at END,
      ended_at=CASE WHEN ${L} THEN ${T} ELSE NULL END
      WHERE kind='${t}' AND id=NEW.id; END;`)}}n(ensureAnalyticsSchema,"ensureAnalyticsSchema"),module.exports={ensureAnalyticsSchema};
