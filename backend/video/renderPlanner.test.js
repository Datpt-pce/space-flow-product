...
function main() {
  ...
  check('Phase 13: caption cue uses an exclusive end boundary and bottom-center position', () => {
    const state = baseState();
    state.tracks.push({
    ...
    const plan = buildRenderPlan(state, { assetPaths: { 'asset-1': '/media/a.mp4' }, outputPath: '/out.mp4', fontFilePath: 'C:\\Windows\\Fonts\\arial.ttf' });
    const filterStr = plan.args[plan.args.indexOf('-filter_complex') + 1];
    assert.ok(filterStr.includes("enable='gte(t\\,1)*lt(t\\,3)'"), filterStr);
    assert.ok(filterStr.includes('y=(h-38.4*1-0.08*h)+0*38.4'), filterStr);
  ...
  check('Phase 13: 2 caption cues -> chained drawtext steps, each with its OWN enable window, in timeline order', () => {
    ...
    assert.ok(filterStr.includes("enable='gte(t\\,1)*lt(t\\,2)'"), filterStr);
    assert.ok(filterStr.includes("enable='gte(t\\,5)*lt(t\\,6)'"), filterStr);
  ...
  check('08-H5: multi-line caption content (real newline, from the CaptionPanel textarea) never embeds a raw \\n inside one drawtext\'s own text= value', () => {
    ...
    // 2nd line stacks BELOW the 1st by a fixed line-height, both sharing the same
    // N-line-aware bottom anchor (see drawtextLineExprs() call site in buildRenderPlan()).
    assert.ok(filterStr.includes('(h-38.4*2-0.08*h)+0*38.4'), filterStr);
    assert.ok(filterStr.includes('(h-38.4*2-0.08*h)+1*38.4'), filterStr);
