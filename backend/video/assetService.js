...
function hashFile(filePath) {
  return new Promise((resolve, reject) => {
    ...
    // Callers may rename immediately after hashing; Windows needs the read handle
    // closed, which occurs after 'end'. Never resolve while our own handle is open.
    stream.on('close', () => digest ? resolve(digest) : reject(new Error('File hash stream closed before completion')));
...
function generateProxy(filePath, outPath, { gopSeconds = 0.5, fps = 30, durationMs = null, crf = 23, maxDimension = null } = {}, onProgress) {
  return new Promise((resolve, reject) => {
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    ...
    const proc = spawn('ffmpeg', [
      '-y',
      '-i', filePath,
      ...(maxDimension ? ['-vf', `scale=w='min(${maxDimension},iw)':h='min(${maxDimension},ih)':force_original_aspect_ratio=decrease:force_divisible_by=2`, '-pix_fmt', 'yuv420p'] : []),
