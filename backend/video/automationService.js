...
function createAutomationService(db, projects, versions) {
  ...
  function materialize(ownerId, creativeId, name, collectionId = null) {
    ...
    if (collectionId && !db.prepare('SELECT id FROM video_timeline_collections WHERE id = ? AND owner_id = ? AND archived_at IS NULL').get(collectionId, ownerId)) throw fail('Không tìm thấy collection.', 404);
    db.prepare('INSERT INTO video_projects(id, owner_id, name, payload, collection_id) VALUES (?, ?, ?, ?, ?)').run(id, ownerId, name.trim(), payload, collectionId);
  ...
  // Internal transaction participant used by Lab. The caller owns the atomic
  // group boundary; public automation operations retain their existing wrapper.
  function materializeDocument(ownerId, document, name, collectionId, source, reusableRecipeId) {
    const slots = document.tracks.flatMap(t => t.clips.map(c => ({ clipId: c.id, trackId: t.id, trackType: t.type, label: `${t.name || t.type} · ${c.id}` })));
    let template = reusableRecipeId && input(ownerId, reusableRecipeId, 'recipe');
    if (!template || canonicalJson(template.payload.slots) !== canonicalJson(slots)) {
      template = saveInput(ownerId, 'recipe', name, { schemaVersion: 1, document, slots, source, dependencies: versions.dependencies(ownerId, document) });
    }
    const components = {};
    for (const track of document.tracks) for (const clip of track.clips) {
      components[clip.id] = saveInput(ownerId, 'component', name, { schemaVersion: 1, trackType: track.type, clip,
        assetHash: clip.assetId ? asset(ownerId, clip.assetId).content_hash : null, source: { ...source, origins: undefined, ...source.origins?.[clip.id], elementId: clip.id } }).id;
    }
    const creative = saveInput(ownerId, 'creative-variant', name, { schemaVersion: 1, recipeVersionId: template.id, components });
    return materialize(ownerId, creative.id, name, collectionId);
  }
  function vectorComponent(ownerId, name, shape) {
    return saveInput(ownerId, 'component', name, { schemaVersion:1, trackType:'video', clip:{ shape }, assetHash:null });
  }
  ...
  function createFromDocument(ownerId, request) {
    return operation(ownerId, request.idempotencyKey, request, () => materializeDocument(ownerId,
      request.document, request.name, null, request.source));
  }
  ...
  return { recipe, createRecipe, listRecipes, input, compile, createVariant, materializeVersion, createFromDocument, context, reset, preview, applySiblings, promoteComponent, undoOperation, materializeDocument, vectorComponent };
}
