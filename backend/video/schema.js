...
function ensureVideoSchema(db) {
  db.exec(`CREATE TABLE IF NOT EXISTS video_batch_speech_jobs (
  ...
  CREATE INDEX IF NOT EXISTS idx_batch_speech_owner_item ON video_batch_speech_jobs(owner_id,project_id,item_id);`);
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS video_batch_capcut_packages (
    id TEXT PRIMARY KEY, owner_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    project_id TEXT NOT NULL REFERENCES video_batch_projects(id) ON DELETE CASCADE,
  ...
  ensureColumn(db, 'video_assets', 'source_machine_id', 'source_machine_id TEXT');
  ensureColumn(db, 'video_assets', 'original_source_path', 'original_source_path TEXT');
  ensureColumn(db, 'video_assets', 'preview_status', "preview_status TEXT CHECK (preview_status IN ('queued','running','done','error'))");
  ensureColumn(db, 'video_assets', 'preview_progress', 'preview_progress REAL');
  ensureColumn(db, 'video_assets', 'preview_error', 'preview_error TEXT');
  db.exec(`
