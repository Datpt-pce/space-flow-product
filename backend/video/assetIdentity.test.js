...
(async () => {
  const rows = await Promise.all(Array.from({ length: 12 }, () => importAsset('owner', 'folder-a/copy.mp3', run, { skipPreflight: true })));
  assert.equal(new Set(rows.map(r => r.id)).size, 1);
  assert.equal(probes, 1, 'concurrent imports of the same source share one probe pipeline');
  assert.equal(db.prepare('SELECT COUNT(*) AS n FROM video_assets').get().n, 1);
  const copy = await importAsset('owner', 'folder-b/copy.mp3', run, { skipPreflight: true });
  assert.notEqual(copy.id, rows[0].id, 'same filename and bytes at different paths stay separate');
  assert.equal(db.prepare('SELECT source_path FROM video_assets WHERE id = ?').get(rows[0].id).source_path, 'folder-a/copy.mp3');
  assert.equal((await importAsset('owner', 'folder-b/copy.mp3', run, { skipPreflight: true })).id, copy.id);
  ...
  console.log('PASS concurrent same-source import; distinct paths, owners and localities retain identity');
