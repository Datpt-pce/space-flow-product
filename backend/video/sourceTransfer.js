...
function createSourceReceiver(root) {
  ...
  return async function receive(kind, p) {
    ...
    // Concurrent deliveries may transfer the same render. Publish once without
    // replacing a cache file another job is already hashing/reading (Windows).
    try { fs.linkSync(files.partial, files.final); }
    catch(e) { if(e.code!=='EEXIST' || fs.statSync(files.final).size!==meta.size || await hashFile(files.final)!==meta.hash) throw e; }
