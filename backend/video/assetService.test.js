...
async function main() {
  ...
  await check('lightweight previews preserve duration/aspect, fit landscape/portrait, never upscale or rewrite source', async () => {
    const { execFileSync } = require('node:child_process');
    const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sf-preview-size-'));
    try {
      for (const [width, height, targetWidth, targetHeight] of [[1920, 1080, 1280, 720], [1080, 1920, 720, 1280], [320, 180, 320, 180]]) {
        const source = path.join(scratchDir, `${width}.mp4`), output = path.join(scratchDir, `${width}-preview.mp4`);
        execFileSync('ffmpeg', ['-v', 'error', '-f', 'lavfi', '-i', `testsrc2=s=${width}x${height}:r=30:d=1.2`, '-c:v', 'libx264', '-preset', 'ultrafast', '-y', source], { windowsHide: true });
        const before = await hashFile(source), metadata = await probeMetadata(source);
        await generateProxy(source, output, { maxDimension: 1280, fps: 30, durationMs: metadata.durationMs });
        const preview = await probeMetadata(output);
        assert.deepStrictEqual([preview.width, preview.height], [targetWidth, targetHeight]);
        assert.ok(Math.abs(preview.durationMs - metadata.durationMs) < 1000 / 30);
        assert.strictEqual(await hashFile(source), before);
      }
