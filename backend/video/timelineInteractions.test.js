...
(async () => {
  ...
  // A minimal image legitimately has fewer families than a Windows workstation.
  assert.ok(families.length > 0, 'enumerate the installed font families');
  assert.ok(families.every(f => typeof f === 'string' && f.trim()));
  assert.equal(new Set(families).size, families.length, 'font picker families are unique');
  if (process.platform === 'linux') assert.ok(families.includes('DejaVu Sans'), 'include the font supplied by the image/CI');
