...
agent.sendJob = async (owner, job, event) => {
  ...
  const result = await runVideoJob(job.kind, job.payload, percent => event('progress', { percent }));
...
(async () => {
  ...
  for (const kind of ['thumbnail', 'proxy']) {
    ...
    const progress = [];
    const result = await run(kind, { path: source, outPath, maxDimension: 1280 }, percent => progress.push(percent));
    assert.equal(result.outPath, outPath);
    assert.ok(fs.statSync(outPath).size > 1000);
    if (kind === 'proxy') assert.equal(progress.at(-1), 100, 'agent progress reaches the preview caller');
  }
