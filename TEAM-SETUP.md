...
- Điền thêm `CREDENTIALS_ENCRYPTION_KEY` gồm **64 ký tự hex** trong `.env` để mã hóa credential.
  Với máy mới chưa có dữ liệu, chạy trong PowerShell rồi chép kết quả vào trường này:
  `[byte[]]$credentialBytes = New-Object byte[] 32; [Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($credentialBytes); -join ($credentialBytes | ForEach-Object { $_.ToString('x2') })`.
  Lưu bản sao key riêng với bản sao dữ liệu. Với máy đã có dữ liệu, giữ/khôi phục **key cũ**;
  không tạo key mới thay key thất lạc vì sẽ không đọc được credential đã mã hóa.
