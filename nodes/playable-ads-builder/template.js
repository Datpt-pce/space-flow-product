var l=Object.defineProperty;var i=(e,t)=>l(e,"name",{value:t,configurable:!0});const NETWORKS={applovin:{extraHead:"",downloadBody:`console.log("IKAME: Download function called");

            if (window.mraid) {
                window.mraid.open(url);
            } else {
                window.open(url, "_blank");
            }`},unity:{extraHead:"",downloadBody:`console.log("IKAME: Download function called");

            if (window.mraid) {
                window.mraid.open(url);
            } else {
                window.open(url, "_blank");
            }`},google:{extraHead:`    <script type="text/javascript" src="https://tpc.googlesyndication.com/pagead/gadgets/html5/api/exitapi.js"> </script>
`,downloadBody:`console.log("IKAME: Download function called");
            ExitApi.exit();`},mintegral:{extraHead:"",downloadBody:`window.install && window.install()
            window.gameEnd && window.gameEnd();`},moloco:{extraHead:"",downloadBody:`console.log("IKAME: Download function called");
            FbPlayableAd.onCTAClick()`}};function escapeHtml(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}i(escapeHtml,"escapeHtml");function buildHtml({title:e,url:t,states:o,base64Video:n,extraHead:a,downloadBody:r}){return`<!DOCTYPE html>
<html>
<head>
...
${a}    <script>
    ...
    <script>
        let mp4_portrait = "data:video/mp4;base64,${n}";let currentOrientation = "";
    </script>
    <script>
        function isTablet() {
        ...
        function handlerLogic() {
          ...
          var _cursor = document.getElementById('builderCursor');
            ...
            _v.loop = false;
            _bg.loop = false;
            var _states = ${JSON.stringify(o,null,4)};
...
</html>`}i(buildHtml,"buildHtml"),module.exports={NETWORKS,buildHtml,escapeHtml};
