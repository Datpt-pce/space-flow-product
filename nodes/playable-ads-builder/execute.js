...
const { MAX_HTML_BYTES, prepareVideo } = require('./compressVideo');
...
module.exports = async function execute(inputs, config, context) {
  ...
  const files_out = [];
  const platforms = [['AND', androidUrl], ['IOS', iosUrl]].filter(([, url]) => url);
  for (let j = 0; j < jobs.length; j++) {
    ...
    const overheadBytes = Math.max(...platforms.flatMap(([, url]) => selectedNetworks.map((net) => (
      Buffer.byteLength(buildHtml({ title, url, states, base64Video: '', ...NETWORKS[net] }), 'utf8')
    ))));
    const video = await prepareVideo(videoPath, overheadBytes, {
      signal: context.signal,
      log: (message) => context.log(message),
      progress: (percent, message) => context.progress(
        Math.round((j + percent / 100) / jobs.length * 95), `${title}: ${message}`,
      ),
    });
    ...
    for (const [platformKey, url] of platforms) {
      ...
      for (const net of selectedNetworks) {
        ...
        if (sizeBytes >= MAX_HTML_BYTES) {
          throw new Error(`Playable ${net}/${platformKey} có ${sizeBytes} byte, phải dưới 5.000.000 byte`);
        }
        context.signal?.throwIfAborted();
        const fileName = `${safeVideoName}_${net}.html`;
        const filePath = path.join(outDir, fileName);
        fs.writeFileSync(filePath, html, 'utf8');
        context.log(`${fileName} (${platformKey}): ${sizeBytes} byte / dưới 5.000.000 byte`);
