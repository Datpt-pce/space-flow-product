...
def _extract_image_url(html):
    for pattern in [
        r'<meta\b[^>]*\b(?:property|name)=["\']og:image["\'][^>]*\bcontent=["\'](https?://[^"\']+)["\']',
        r'<meta\b[^>]*\bcontent=["\'](https?://[^"\']+)["\'][^>]*\b(?:property|name)=["\']og:image["\']',
...
def download_pinterest_image(url, out_dir):
    ...
    raw_img_url = _extract_image_url(resp.text)
    if not raw_img_url:
    ...
    ext_m = re.search(r'\.(jpg|jpeg|png|gif|webp)', raw_img_url, re.IGNORECASE)
    ext = (ext_m.group(1).lower() if ext_m else 'jpg').replace('jpeg', 'jpg')

    file_path = os.path.join(out_dir, f'pin_{pin_id}.{ext}')
    # Thử bản /originals/ (độ phân giải đầy đủ) trước; một số pin không có bản originals
    # trên CDN (S3 trả 403 AccessDenied dù URL đúng định dạng) — khi đó fallback về đúng
    # URL độ phân giải đã lấy được từ trang (vd. /736x/), vẫn tải được bình thường.
    originals_url = re.sub(r'/\d+x/', '/originals/', raw_img_url)
    img_resp = requests.get(originals_url, headers=headers, timeout=30, stream=True)
    if img_resp.status_code == 403 and originals_url != raw_img_url:
        img_resp = requests.get(raw_img_url, headers=headers, timeout=30, stream=True)
    ...
    try:
        ...
        for backoff in (0, 3, 8, 15, 25):
            ...
            try:
                ...
                last_err = e
                # Lỗi kiểu "không có video" là vĩnh viễn (pin chỉ có ảnh) — retry không giúp
                # gì, dừng ngay để rơi xuống nhánh fallback ảnh thay vì đợi hết cả 5 lần backoff.
                if any(k in str(e).lower() for k in _no_video_kw):
                    break
