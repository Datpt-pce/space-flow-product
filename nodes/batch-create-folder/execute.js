...
module.exports = async function execute(inputs, config, context) {
  ...
  if (Array.isArray(inputs?.items_in) && inputs.items_in.length > 0) {
    rows = fromItems(inputs.items_in).map(item =>
      typeof item === 'string'
        ? { text: item, variants: [] }
  ...
  if (config?.create_folders) {
    const basePath = toContainerPath((config?.base_path || '').trim());
  ...
  // `paths` has no declared output port (node.json) — left as-is, unused by the graph.
  return { names: toItems(names), paths };
};
