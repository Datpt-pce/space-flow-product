...
def validate_package(package):
    ...
    if report.get('format') != 'draft-content-v1' or report.get('mode') != 'editable-batch':
    ...
    if report.get('mode') == 'editable-batch':
        from batch_capcut import validate_batch_package
        validate_batch_package(project, report)
        return report
...
def install_package(package, drafts_root, *, qualification=False):
    ...
    if report.get('format') == 'draft-content-v1' and report.get('mode') == 'editable-batch':
        if capcut_running() or (package / 'installation-live.json').exists():
            from capcut_live_install import install_live
            return install_live(package, drafts_root, report)
    if report.get('format') != 'draft-content-v1' or report.get('mode') != 'editable-batch':
    ...
    try:
        ...
        write_json(package / 'installation.json', {
            ...
            'projectId': report['projectId'], 'appBuild': report.get('appBuild'),
        })
...
def main():
    ...
    package_root = Path(os.environ.get('SF_CAPCUT_PACKAGE_DIR') or Path(__file__).resolve().parents[1] / '../backend/uploads/capcut-packages')
    ...
    if operation == 'prepare-batch':
        ...
        return prepare_batch_package(payload, package_root)
    if operation in ('install', 'render-start', 'render-status'):
        ...
        if operation == 'install': return install_package(package, drafts)
        from capcut_render import start, status
        if operation == 'render-status': return status(package, payload['requestKey'])
        return start(package, payload['requestKey'], payload.get('timelineIds', payload.get('timelineId')), payload['outputDir'], drafts)
