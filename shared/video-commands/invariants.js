...
function assertTransitionsReferenceAdjacentClips(state) {
  for (const transition of state.transitions || []) {
    ...
    if (transition.type && !require('../video-transition-catalog.json').some(t => t.type === transition.type)) throw new Error('Loại transition không hỗ trợ');
    if (transition.timingMode !== undefined && transition.timingMode !== 'centered-v1') throw new Error('Transition timing mode không hỗ trợ');
    if (transition.timingMode === 'centered-v1') {
      const frameCount = transition.durationMs * (state.fps || 30) / 1000;
      if (Math.abs(frameCount - Math.round(frameCount)) > 1e-6) throw new Error('Transition cần duration nguyên frame.');
      const { transitionWindow } = require('../video-transition-timing');
      const window = transitionWindow(transition, toClip.timelineInMs, state.fps || 30);
      for (const other of state.transitions || []) {
        if (other.id === transition.id || other.timingMode !== 'centered-v1' || other.fromClipId !== toClip.id) continue;
        const next = state.tracks.flatMap(t=>t.clips).find(c=>c.id===other.toClipId);
        if (next && window.startMs + window.durationMs > transitionWindow(other, next.timelineInMs, state.fps || 30).startMs + 1e-6) throw new Error('Hai transition chiếm chồng thời gian trên clip giữa.');
      }
    }
  }
}
