...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      appearanceSettings: { designMode: 'lts', theme: 'light' },
      ...
      runWorkflow: async (startNodeId = null, resume = false) => {
        ...
        set({ executionLogs: [], nodeStatuses: {}, nodeErrorInfo: {}, nodeOutputs: {}, nodeProgress: {}, isRunning: true, isLogOpen: true });
        ...
        try {
          await executeWorkflow(workflow, (eventType, data) => {
            if (eventType === 'log') {
              ...
              set({ isLogOpen: true });
              // Mark the currently-running node(s) as a hard-stop error (workflow halted)
              set(s => {
      ...
      // Model: config.textItems: [{id,text}], config.itemOrder: ['file:<idx>'|'text:<id>', ...],
      // config.itemMode: 'append'|'replace'. itemOrder absent/rỗng = thứ tự mặc định (files rồi
      // textItems) — xem nodes/list/execute.js cho logic đọc lại giống hệt.
      _listDefaultOrder: (config) => {
        const files = (config?.files || []).filter(f => typeof f === 'string');
        const textItems = config?.textItems || [];
        return [...files.map((_, i) => `file:${i}`), ...textItems.map(t => `text:${t.id}`)];
      },
      ...
      removeListItemBeta: (nodeId, token) => {
        set(s => ({
          nodes: s.nodes.map(n => {
            ...
            return { ...n, data: { ...n.data, config: { ...config, files, textItems, itemOrder: order } } };
          }),
        }));
        get().cleanupOrphanedFiles();
      },
      ...
      saveCurrentToLibrary: async (name, visibility) => {
        ...
        return createWorkflow(name, visibility, payload);
      },
