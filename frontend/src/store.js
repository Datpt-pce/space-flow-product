...
import { withNodeHistory, preserveCurrentConfigs } from './store/nodeHistory.js';
import { create } from 'zustand';
import { persist } from 'zustand/middleware';
import { applyNodeChanges, applyEdgeChanges } from '@xyflow/react';
import { cleanupFiles, browseFolder, browseFile, createWorkflow, updateWorkflow, loadWorkflow as apiLoadWorkflow } from './lib/api.js';
import { observeRun, cancelRun, runNodeIds } from './lib/workflowRuns.js';
import { track, trackConfig } from './lib/analyticsClient.js';
import { itemToPath } from './lib/items.js';
import { prepareResizeDriveInputs, queuedDriveFolders } from './lib/resizeDrivePreparation.js';
import { retainResizeOutput, persistedResizeOutputs, restoredResizeOutputs } from './lib/resizeRetainedOutputs.js';

const MAX_HISTORY = 50;
const runObservers = new Map();

export const useStore = create(
  persist(
    withNodeHistory((set, get) => ({
      nodes: [],
      edges: [],
      nodeManifests: {},
      ...
      retainedResizeOutputs: {}, // Browser-persisted Resize results, keyed by node and owner.
      ...
      resizeDriveRequest: null,
      prepareResizeDriveInputs: nodeId => prepareResizeDriveInputs(nodeId, get, set),
      ...
      openSettings: (tab) => set({ isSettingsOpen: true, settingsInitialTab: typeof tab === 'string' ? tab : null }),
      ...
      undo: () => {
        ...
        set({
          nodes: preserveCurrentConfigs(prev.nodes, s.nodes),
          edges: prev.edges,
      ...
      redo: () => {
        ...
        const current = { nodes: s.nodes, edges: s.edges };
        set({
          nodes: preserveCurrentConfigs(next.nodes, s.nodes),
          edges: next.edges,
      ...
      addEdge: (edge) => {
        track('connection_added', { target: edge.target }, { nodeType: get().nodes.find(n => n.id === edge.target)?.type });
        get()._pushUndo();
      ...
      updateNodeConfig: (nodeId, key, value) => {
        trackConfig(nodeId, get().nodes.find(n => n.id === nodeId)?.type);
        set(s => ({
          nodes: s.nodes.map(n =>
            n.id === nodeId
      ...
      addNodeToCanvas: (manifest, position, configOverrides = {}) => {
        track('node_added', {}, { nodeType: manifest.id });
        get()._pushUndo();
      ...
      reconnectRuns: () => {
        ...
        for (const [key, observer] of runObservers) {
          if (observer.ownerId !== ownerId) { observer.controller.abort(); runObservers.delete(key); }
        }
        const runs = Object.values(get().activeRuns).filter(run => run.ownerId === ownerId);
        set({ activeRunCount: runs.length, isRunning: runs.length > 0,
          nodeStatuses: {}, nodeOutputs: restoredResizeOutputs(get()), nodeProgress: {}, nodeErrorInfo: {}, executionLogs: [] });
        for (const run of runs) void get().runWorkflow(null, false, run);
      },

      cancelTrackedRun: async (key) => {
        const run = get().activeRuns[key];
        if (!run || run.ownerId !== get().currentUser?.id) return;
        set(s => ({ activeRuns: { ...s.activeRuns, [key]: { ...s.activeRuns[key], cancelRequested: true } } }));
        if (!run.id) return; // Admission still in flight; onCreated delivers the pending STOP.
        try { await cancelRun(run.id); }
        catch (error) {
          set(s => ({ activeRuns: s.activeRuns[key]
            ? { ...s.activeRuns, [key]: { ...s.activeRuns[key], cancelRequested: false } } : s.activeRuns,
          executionLogs: [...s.executionLogs, { ts: Date.now(), message: 'STOP: ' + error.message, level: 'error' }], isLogOpen: true }));
        }
      },

      stopRuns: async (nodeId = null) => {
        const preparation = get().resizeDriveRequest;
        if (preparation && (!nodeId || preparation.nodeId === nodeId)) preparation.finish(null);
        track('cancel_clicked', nodeId ? { target: nodeId } : {}, { nodeType: get().nodes.find(n => n.id === nodeId)?.type });
        const runs = Object.values(get().activeRuns).filter(run =>
          run.ownerId === get().currentUser?.id && (!nodeId || run.nodeIds.includes(nodeId)));
        for (const run of runs) {
          await get().cancelTrackedRun(run.key);
        }
      },

      runWorkflow: async (startNodeId = null, resume = false, existingRun = null, direct = null) => {
        const ownerId = get().currentUser?.id;
        if (!ownerId || (existingRun && runObservers.has(existingRun.key))) return;
        if (!existingRun) {
          const payload = get().buildWorkflowPayload();
          const scope = direct ? [direct.node.id] : runNodeIds(payload, startNodeId);
          try {
            for (const node of payload.nodes.filter(node => scope.includes(node.id) && (direct || (node.active !== false && !node.pinnedData)))) {
              if (!queuedDriveFolders(node.type, node.config).length) continue;
              if (!await get().prepareResizeDriveInputs(node.id)) return;
            }
            if (get().currentUser?.id !== ownerId) return;
            if (direct) direct = { ...direct, node: get().nodes.find(node => node.id === direct.node.id) };
          } catch (error) {
            set(s => ({ executionLogs: [...s.executionLogs, { ts: Date.now(), message: error.message, level: 'error' }], isLogOpen: true }));
            return;
          }
        }
        if (!existingRun) track('run_clicked', startNodeId ? { target: startNodeId } : {}, { nodeType: direct?.node.type || get().nodes.find(n => n.id === startNodeId)?.type });
        const workflow = existingRun?.intent?.workflow || (direct ? {
          nodes: [{ id: direct.node.id, type: direct.node.type,
            config: { ...direct.node.data.config, run_mode: direct.mode }, mockInput: direct.inputs }], edges: [],
        } : get().buildWorkflowPayload());
        const run = existingRun || { key: crypto.randomUUID(), ownerId,
          nodeIds: runNodeIds(workflow, startNodeId), directNodeId: direct?.node.id,
          intent: { workflow, startNodeId, resume } };
        const controller = new AbortController();
        runObservers.set(run.key, { controller, ownerId });
        set(s => ({ activeRuns: { ...s.activeRuns, [run.key]: run } }));
        if (!existingRun) {
          const rerunIds = new Set(workflow.nodes.filter(node => run.nodeIds.includes(node.id) && node.active !== false && !node.pinnedData).map(node => node.id));
          set(s => ({
            retainedResizeOutputs: Object.fromEntries(Object.entries(s.retainedResizeOutputs).filter(([id]) => !rerunIds.has(id))),
            nodeOutputs: Object.fromEntries(Object.entries(s.nodeOutputs).filter(([id]) => !rerunIds.has(id))),
          }));
        }
        if (run.id && run.cancelRequested) void get().cancelTrackedRun(run.key);
        ...
        set(s => ({
          ...
          ...(isFirstRun && !existingRun
            ? { executionLogs: [], nodeStatuses: {}, nodeErrorInfo: {}, nodeOutputs: restoredResizeOutputs(s), nodeProgress: {} }
            : {}),
          activeRunCount: Object.values(s.activeRuns).filter(run => run.ownerId === ownerId).length,
          isRunning: true,
          isLogOpen: true,
        }));
        ...
        const touchedNodeIds = new Set(run.directNodeId ? [run.directNodeId] : []);
        if (run.directNodeId && !existingRun) set(s => ({ nodeStatuses: { ...s.nodeStatuses, [run.directNodeId]: 'running' },
          nodeOutputs: { ...s.nodeOutputs, [run.directNodeId]: {} }, nodeProgress: { ...s.nodeProgress, [run.directNodeId]: undefined } }));

        const addLog = (nodeId, message, level = 'info') =>
          set(s => ({ executionLogs: [...s.executionLogs, { ts: Date.now(), nodeId, message, level }] }));

        try {
          await observeRun(run, (eventType, data) => {
            if (controller.signal.aborted || get().currentUser?.id !== ownerId) return;
            if (eventType === 'nodeStart') set(s => ({ nodeStatuses: { ...s.nodeStatuses, [data.nodeId]: 'running' } }));
            ...
            if (eventType === 'log') {
              ...
              set(s => {
                ...
                const output = { ...existing, rows: { ...(existing.rows || {}), [data.row_id]: data } };
                return {
                  retainedResizeOutputs: retainResizeOutput(s, nodeId, output),
                  nodeOutputs: {
                    ...s.nodeOutputs,
                    [nodeId]: output,
                  },
              ...
              const newStatus = skippedExec ? 'idle' : outputs.success === false ? 'error' : 'done';
              set(s => {
                ...
                return {
                  ...
                  retainedResizeOutputs: skippedExec ? s.retainedResizeOutputs : retainResizeOutput(s, nodeId, outputs),
                  nodeProgress,
                };
              });
              ...
              if (!run.directNodeId && srcNode && srcManifest?.outputs) {
              ...
              set(s => {
                ...
                for (const id of touchedNodeIds) {
                  if (updated[id] === 'running') {
                    updated[id] = data.state === 'cancelled' ? 'idle' : 'error';
                    errorInfo[id] = { continued: false };
                  }
                  delete nodeProgress[id];
                }
                return { nodeStatuses: updated, nodeErrorInfo: errorInfo, nodeProgress,
                  ...(run.directNodeId ? { nodeOutputs: { ...s.nodeOutputs,
                    [run.directNodeId]: { ...s.nodeOutputs[run.directNodeId], error: data.error } } } : {}) };
              });
            ...
            set(s => ({ activeRuns: { ...s.activeRuns, [run.key]: { ...s.activeRuns[run.key], id, intent: undefined } } }));
            if (get().activeRuns[run.key]?.cancelRequested) await get().cancelTrackedRun(run.key);
          }, controller.signal);
        } catch (err) {
          if (controller.signal.aborted || get().currentUser?.id !== ownerId) return;
          ...
          set(s => {
            ...
            return { nodeStatuses: updated, nodeErrorInfo: errorInfo, nodeProgress, isLogOpen: true,
              ...(run.directNodeId ? { nodeOutputs: { ...s.nodeOutputs,
                [run.directNodeId]: { ...s.nodeOutputs[run.directNodeId], error: err.message } } } : {}) };
          });
        }

        if (runObservers.get(run.key)?.controller === controller) runObservers.delete(run.key);
        if (controller.signal.aborted) return;
        set(s => {
          const activeRuns = { ...s.activeRuns };
          delete activeRuns[run.key];
          const activeRunCount = Object.values(activeRuns).filter(run => run.ownerId === s.currentUser?.id).length;
          return { activeRuns, activeRunCount, isRunning: activeRunCount > 0 };
        });
      },

      continueWorkflow: (nodeId) => get().runWorkflow(nodeId, true),

      runDirectNode: (nodeId, mode, inputs = {}) => {
        const node = get().nodes.find(node => node.id === nodeId);
        if (!node || Object.values(get().activeRuns).some(run =>
          run.ownerId === get().currentUser?.id && run.nodeIds.includes(nodeId))) return;
        return get().runWorkflow(null, false, null, { node, mode, inputs });
      },
      runNms: (nodeId, mode) => get().runDirectNode(nodeId, mode),
      runNmsV2: (nodeId, mode) => get().runDirectNode(nodeId, mode),
      runNmsV3: (nodeId, mode = 'full', inputs = {}) => get().runDirectNode(nodeId, mode, inputs),

      deleteNode: (nodeId) => {
        track('node_deleted', { target: nodeId }, { nodeType: get().nodes.find(n => n.id === nodeId)?.type });
        get()._pushUndo();
        set(s => ({
    ...
    })),
    {
      ...
      partialize: (s) => ({
        retainedResizeOutputs: persistedResizeOutputs(s),
