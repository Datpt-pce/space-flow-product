...
import {
  ...
  asanaTasks, asanaInspect, asanaTasksV2,
} from '../lib/api.js';
export { resolveDropPaths } from '../lib/dropResolve.js';

export const inputCls = "w-full h-7 px-2 rounded-md border border-[var(--card-border,#e5e7eb)] text-[11px] text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-red-400";
export const labelCls = "text-[9px] text-[var(--n400,#9ca3af)] uppercase tracking-wide mb-1";
...
export function CopyButton({ text }) {
  ...
  return (
    <button
      className="nodrag flex-shrink-0 px-1.5 py-0.5 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--n500,#6b7280)] hover:bg-[var(--n200,#e5e7eb)] transition-colors"
...
export function Modal({ title, onClose, width = 480, children }) {
  return (
    <div className="nodrag fixed inset-0 z-[10000] flex items-center justify-center bg-black/30" onClick={onClose}>
      <div
        className="bg-[var(--card,#fff)] rounded-2xl shadow-2xl border border-[var(--card-border,#e5e7eb)] max-h-[80vh] flex flex-col"
        style={{ width }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--card-border,#f3f4f6)]">
          <span className="text-[12px] font-semibold text-[var(--sub,#374151)]">{title}</span>
          <button className="p-1 rounded-md hover:bg-[var(--n100,#f3f4f6)] text-[var(--n400,#9ca3af)]" onClick={onClose}><X size={14} /></button>
        </div>
...
export function AppManagerModal({ apps, onClose, onReload, onSaveApp = saveResizeUploadApp, onDeleteApp = deleteResizeUploadApp, title = 'Quản lý App (UNC)' }) {
  ...
  const save = async () => {
    if (!name.trim()) return;
    await onSaveApp({ tag: name.trim(), original_tag: selectedTag, folder, thumbnail_folder: thumbFolder, pinned });
    onReload();
    setSelectedTag(name.trim());
  };
  const remove = async () => {
    if (!selectedTag) return;
    await onDeleteApp(selectedTag);
  ...
  const duplicate = async () => {
    ...
    await onSaveApp({ tag: newTag, folder: apps[selectedTag].folder, thumbnail_folder: apps[selectedTag].thumbnail_folder, pinned: apps[selectedTag].pinned });
    onReload();
  };
  const handlePickFolder = async (setter) => { const path = await pickFolder(); if (path) setter(path); };

  return (
    <Modal title={title} width={620} onClose={onClose}>
      <div className="flex gap-3">
        <div className="w-40 flex-shrink-0 border border-[var(--card-border,#f3f4f6)] rounded-lg max-h-64 overflow-y-auto">
          {tags.map(t => (
            <div key={t} onClick={() => select(t)} className={`px-2 py-1 text-[10px] cursor-pointer truncate ${selectedTag === t ? 'bg-red-50 text-red-700' : 'hover:bg-[var(--n50,#f9fafb)] text-[var(--sub,#4b5563)]'}`}>
              {apps[t]?.pinned ? '⭐ ' : ''}{t}
            </div>
          ))}
        </div>
        <div className="flex-1 flex flex-col gap-2">
          ...
          <div className="flex gap-1 mt-1">
            ...
            <button className="flex-1 py-1.5 rounded-md bg-[var(--n100,#f3f4f6)] text-[10px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={duplicate}>Nhân bản</button>
...
export function ChannelManagerModal({ settings, onSave, onClose }) {
  ...
  return (
    <Modal title="Quản lý Kênh (GCS Google Parent)" width={360} onClose={onClose}>
      <div className="border border-[var(--card-border,#f3f4f6)] rounded-lg max-h-48 overflow-y-auto mb-2">
        {channels.map(c => (
          <div key={c} className="flex items-center justify-between px-2 py-1 hover:bg-[var(--n50,#f9fafb)]">
            <span className="text-[10px] text-[var(--sub,#4b5563)]">{c}</span>
            <button onClick={() => remove(c)} className="text-[var(--n300,#d1d5db)] hover:text-red-500"><X size={11} /></button>
          </div>
        ))}
      </div>
      <div className="flex gap-1">
...
export function TaskSelectorModal({ pat, credentialName, onInsert, onClose, usedUrls = {} }) {
  const [tasks, setTasks] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState(new Set());

  // credentialName !== undefined → V2 (resolve credential server-side); otherwise V1's raw pat.
  const hasSource = credentialName !== undefined ? !!credentialName : !!pat;

  useEffect(() => {
    if (!hasSource) { setLoading(false); return; }
    const fetchTasks = credentialName !== undefined ? asanaTasksV2(credentialName) : asanaTasks(pat);
    fetchTasks.then(r => { setTasks(r.tasks || []); setLoading(false); });
  }, [pat, credentialName]);
  ...
  return (
    <Modal title="Chọn Task từ Asana (7 ngày gần nhất, chưa xong)" width={500} onClose={onClose}>
      {!hasSource && <div className="text-[10px] text-[var(--n400,#9ca3af)]">{credentialName !== undefined ? 'Chọn Asana credential trước.' : 'Nhập Asana PAT trước.'}</div>}
      {hasSource && loading && <div className="text-[10px] text-[var(--n400,#9ca3af)]">Đang tải...</div>}
      {hasSource && !loading && tasks.length === 0 && <div className="text-[10px] text-[var(--n400,#9ca3af)]">Không tìm thấy task nào.</div>}
      <div className="max-h-72 overflow-y-auto">
        {tasks.map((t, i) => {
          const usedRow = usedUrls[t.permalink_url];
          return (
            <label key={i} className={`flex items-center gap-2 px-1 py-1 ${usedRow ? 'opacity-40 cursor-not-allowed' : 'hover:bg-[var(--n50,#f9fafb)] cursor-pointer'}`}>
              ...
              <span className="text-[10px] text-[var(--sub,#4b5563)] truncate">[{t.project}] {t.name}</span>
...
export function InspectFieldsModal({ pat, taskUrl, onClose }) {
  ...
  return (
    <Modal title="Custom Fields của Task" width={560} onClose={onClose}>
      ...
      {data && (
        <div className="text-[10px] text-[var(--sub,#4b5563)] font-mono whitespace-pre-wrap">
          ...
          {data.project_fields.map((f, i) => (
            <div key={i} className="mb-1">
              {f.name} (GID: {f.gid})
              {f.options.map((o, j) => <div key={j} className="pl-3 text-[var(--n400,#9ca3af)]">- {o.name} (GID: {o.gid})</div>)}
            </div>
          ))}
        </div>
      )}
