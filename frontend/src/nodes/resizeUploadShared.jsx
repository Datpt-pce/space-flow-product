...
export function TaskSelectorModal({ pat, onInsert, onClose, usedUrls = {} }) {
  ...
  return (
    <Modal title="Chọn Task từ Asana (7 ngày gần nhất, chưa xong)" width={500} onClose={onClose}>
      {!pat && <div className="text-[10px] text-[var(--n400,#9ca3af)]">Nhập Asana PAT trước.</div>}
      {pat && loading && <div className="text-[10px] text-[var(--n400,#9ca3af)]">Đang tải...</div>}
      {pat && !loading && tasks.length === 0 && <div className="text-[10px] text-[var(--n400,#9ca3af)]">Không tìm thấy task nào.</div>}
