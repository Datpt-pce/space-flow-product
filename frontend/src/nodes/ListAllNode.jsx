      ...
      <NodeToolbar isVisible={!!selected} position={Position.Top} align="start" offset={8}>
        <div className="flex items-center gap-0.5 bg-[var(--card,#fff)] rounded-2xl shadow-lg border border-[var(--card-border,#e5e7eb)] px-1.5 py-1.5">
          ...
          {effectiveMode === 'files' && files.length > 0 && (
            <>
              <div className="w-px h-4 bg-[var(--n200,#e5e7eb)]" />
              <button className="p-1.5 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors text-[var(--n500,#6b7280)]" title="Mở thư mục" onClick={() => openFolder(files[0]).then(r => { if (r?.error) alert(r.error); }).catch(() => {})}>
                <FolderOpen size={12} />
              </button>
              <button className="p-1.5 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors text-[var(--n500,#6b7280)]" title="Tải toàn bộ (zip)" onClick={() => downloadZip(files).catch(() => {})}>
                <Download size={12} />
              </button>
            </>
          )}
      ...
      >
        ...
        <div
          ...
          className={`relative w-full rounded-2xl overflow-hidden flex flex-col outline-none transition-colors flex-1 min-h-0 ${
            isDragOver ? 'bg-teal-50' : isFocused ? 'bg-blue-50' : 'bg-[var(--n50,#f9fafb)]'
          }`}
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 pt-2.5 pb-1.5 flex-shrink-0">
            <span className="text-[10px] text-[var(--n400,#9ca3af)]">
              {itemCount > 0
            ...
            <div className="flex items-center gap-1 nodrag">
              ...
              {!isConnected && (
                <div className="flex items-center bg-[var(--card,#fff)] rounded-lg border border-[var(--card-border,#e5e7eb)] p-0.5">
                  <button
                    className={`p-1 rounded-md transition-colors ${config.mode === 'text' || !config.mode ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
                  ...
                  <button
                    className={`p-1 rounded-md transition-colors ${config.mode === 'table' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
                  ...
                  <button
                    className={`p-1 rounded-md transition-colors ${config.mode === 'files' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
              ...
              {effectiveMode === 'files' && (
                <div className="flex items-center bg-[var(--card,#fff)] rounded-lg border border-[var(--card-border,#e5e7eb)] p-0.5">
                  <button
                    className={`p-1 rounded-md transition-colors ${view === 'list' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
                  ...
                  <button
                    className={`p-1 rounded-md transition-colors ${view === 'grid' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
              ...
              {!isEmpty && (
                <button
                  className="p-1 rounded-md text-[var(--n400,#9ca3af)] hover:text-red-400 hover:bg-red-50 transition-colors"
                  onClick={handleClear}
                  title="Xóa tất cả"
                >
                  <X size={11} />
          ...
          {!isRunning && isEmpty && effectiveMode === 'files' && (
            <div className="flex-1 flex items-center justify-center px-4">
              <span className="text-[11px] text-[var(--n300,#d1d5db)] text-center leading-relaxed">
          ...
          {!isRunning && isEmpty && isTextMode && (
            ...
            >
              {isConnected ? (
                <span className="text-[11px] text-[var(--n300,#d1d5db)] text-center">Run để xem kết quả</span>
              ) : isFocused ? (
                <span className="text-[11px] text-blue-400 font-medium animate-pulse">Nhấn Ctrl+V để paste</span>
              ) : (
                <>
                  <span className="text-[11px] text-[var(--n300,#d1d5db)] text-center">Click vào đây rồi Ctrl+V</span>
                  <span className="text-[10px] text-[var(--n300,#d1d5db)] text-center">Nhận diện bảng tự động</span>
                </>
              )}
            </div>
          )}
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
              {files.map((f, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-[var(--card,#fff)] transition-colors cursor-default"
                  ...
                  >
                    <Icon size={12} className="text-[var(--n400,#9ca3af)] flex-shrink-0" />
                    <span className="text-[11px] text-[var(--sub,#4b5563)] truncate">{displayName(f)}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-2">
              <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(auto-fill, 48px)' }}>
                {files.map((f, i) => (
                  <div
                    key={i}
                    className="rounded-md overflow-hidden bg-[var(--n200,#e5e7eb)] relative cursor-default"
                  ...
                  >
                    {isImage(f) ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        {(() => { const Icon = fileIcon(f); return <Icon size={14} className="text-[var(--n400,#9ca3af)]" />; })()}
                      </div>
                    )}
                  </div>
                ))}
          ...
          {!isRunning && !isEmpty && effectiveMode === 'text' && (
            <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
              {items.map((item, i) => (
                <div key={i} className="flex items-start gap-2 px-2 py-0.5 rounded-lg hover:bg-[var(--card,#fff)] transition-colors">
                  <span className="text-[9px] text-[var(--n300,#d1d5db)] font-mono w-4 text-right flex-shrink-0 mt-0.5 select-none">{i + 1}</span>
                  <span className="text-[11px] text-[var(--sub,#374151)] leading-relaxed break-all">{item}</span>
                </div>
              ))}
            </div>
          )}
                ...
                <thead>
                  <tr>
                    {headers.slice(0, TABLE_COL_MAX).map((h, i) => (
                      <th key={i} className="text-left px-2 py-1 bg-[var(--card,#fff)] text-[var(--n500,#6b7280)] font-semibold border-b border-[var(--card-border,#e5e7eb)] truncate" style={{ maxWidth: 72 }}>
                        {h || `Col${i + 1}`}
                      </th>
                    ))}
                    {headers.length > TABLE_COL_MAX && (
                      <th className="px-2 py-1 bg-[var(--card,#fff)] text-[var(--n300,#d1d5db)] font-normal text-center">+{headers.length - TABLE_COL_MAX}</th>
                    )}
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row, ri) => (
                    <tr key={ri} className="border-b border-[var(--card-border,#f3f4f6)] hover:bg-[var(--card,#fff)] transition-colors">
                      {row.slice(0, TABLE_COL_MAX).map((cell, ci) => (
                        <td key={ci} className="px-2 py-1 text-[var(--sub,#4b5563)] truncate" style={{ maxWidth: 72 }}>{cell}</td>
                      ))}
                      {row.length > TABLE_COL_MAX && (
                        <td className="px-2 py-1 text-[var(--n300,#d1d5db)] text-center">…</td>
                      )}
                    </tr>
                  ))}
                </tbody>
