  ...
  return (
    <div className="relative flex flex-col" style={{ width: nodeW, height: '100%', opacity: isActive ? 1 : 0.4 }}>
      ...
      >
        ...
        <div className="rounded-2xl bg-[var(--n50,#f9fafb)] p-3 overflow-y-auto flex-1 min-h-0 nodrag">
          <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(3, minmax(0,1fr))' }}>

            {/* ===== Cột 1: Input/Output, Resize, Nền ===== */}
            <div className="flex flex-col gap-2.5 min-w-0">
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                <div className={labelCls}>1) Input & Output</div>
                <div
                  className="rounded-lg border border-dashed border-[var(--card-border,#e5e7eb)] p-1.5 mb-1.5 max-h-[90px] overflow-y-auto"
                ...
                >
                  {folders.length === 0 && <div className="text-[9px] text-[var(--n400,#9ca3af)] text-center py-2">Kéo thả thư mục vào đây</div>}
                  {folders.map((p, i) => (
                    <div key={i} className="flex items-center gap-1 px-1.5 py-0.5 rounded-md hover:bg-[var(--n50,#f9fafb)]">
                      <Folder size={9} className="text-[var(--n400,#9ca3af)] flex-shrink-0" />
                      <span className="text-[9px] text-[var(--sub,#4b5563)] truncate flex-1" title={p}>{basename(p)}</span>
                      <button onClick={() => removeFolder(i)} className="flex-shrink-0 text-[var(--n300,#d1d5db)] hover:text-red-500"><X size={9} /></button>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-1 mb-2">
                  <input
                    type="text"
                    className="flex-1 h-6 px-1.5 rounded-md border border-[var(--card-border,#e5e7eb)] text-[9px] text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-red-400"
              ...
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                ...
                <div className="flex flex-col gap-1 mb-1.5">
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-[var(--sub,#4b5563)] truncate flex-1" title={cfg('cred_config_path', '')}>{basename(cfg('cred_config_path', '')) || 'config222.json'}</span>
                    <button className="flex-shrink-0 p-1 rounded-md bg-[var(--n100,#f3f4f6)] hover:bg-[var(--n200,#e5e7eb)] text-[var(--n500,#6b7280)]" title="Đổi đường dẫn (browse, chỉ dùng khi chạy dev Windows)" onClick={() => handleBrowseCredPath('cred_config_path')}><FolderOpen size={10} /></button>
                    <label className="flex-shrink-0 p-1 rounded-md bg-[var(--n100,#f3f4f6)] hover:bg-[var(--n200,#e5e7eb)] text-[var(--n500,#6b7280)] cursor-pointer" title="Upload file (dùng được cả trên bản product/Docker)">
                      <FileUp size={10} />
                      <input type="file" accept=".json" className="hidden" onChange={e => handleUploadCredPath('cred_config_path', e.target.files[0])} />
                    </label>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-[var(--sub,#4b5563)] truncate flex-1" title={cfg('cred_links_path', '')}>{basename(cfg('cred_links_path', '')) || 'custom_links.json'}</span>
                    <button className="flex-shrink-0 p-1 rounded-md bg-[var(--n100,#f3f4f6)] hover:bg-[var(--n200,#e5e7eb)] text-[var(--n500,#6b7280)]" title="Đổi đường dẫn (browse, chỉ dùng khi chạy dev Windows)" onClick={() => handleBrowseCredPath('cred_links_path')}><FolderOpen size={10} /></button>
                    <label className="flex-shrink-0 p-1 rounded-md bg-[var(--n100,#f3f4f6)] hover:bg-[var(--n200,#e5e7eb)] text-[var(--n500,#6b7280)] cursor-pointer" title="Upload file (dùng được cả trên bản product/Docker)">
                      <FileUp size={10} />
                ...
                <button className="w-full py-1 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)] transition-colors" onClick={handleLoadCredentials}>Load</button>
              </div>
              ...
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                ...
                <div className="flex flex-col gap-1 mb-2">
                  {MODES.map(m => (
                    <label key={m.value} className="flex items-center gap-1.5 cursor-pointer">
                      ...
                      <span className="text-[10px] text-[var(--sub,#4b5563)]">{m.label}</span>
                    </label>
                  ))}
                </div>
                <label className="flex items-center gap-1.5 cursor-pointer mb-1">
                  ...
                  <span className="text-[10px] text-[var(--sub,#4b5563)]">Đổi tên (Theme_CodeApp_Language)</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer">
                  ...
                  <span className="text-[10px] text-[var(--sub,#4b5563)]">Xuất thumbnail JPG</span>
                </label>
              </div>
            ...
            <div className="flex flex-col gap-2.5 min-w-0">
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                <label className="flex items-center gap-1.5 cursor-pointer mb-1.5">
                  ...
                  <span className="text-[10px] font-semibold text-[var(--sub,#374151)]">4) Asana</span>
                </label>
                {cfg('use_asana', true) && (
                  <div className="flex flex-col gap-1.5">
                    <textarea
                      className="w-full h-12 px-2 py-1 rounded-md border border-[var(--card-border,#e5e7eb)] text-[9px] text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-red-400 resize-none"
                      placeholder="Task URLs (mỗi dòng 1 task)"
                      value={cfg('task_urls', '')} onChange={e => set('task_urls', e.target.value)}
                      onMouseDown={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()}
                    />
                    <div className="flex gap-1">
                      <button className="flex-1 py-1 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={() => setTaskSelectorOpen(true)}>Chọn Task…</button>
                      <button className="flex-1 py-1 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={() => setInspectOpen(true)}>Xem Fields</button>
                    </div>
                    <div className="flex gap-1">
                    ...
                    <button className="py-1 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={handleAutoGid}>Tự động tìm GID</button>
              ...
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                <label className="flex items-center gap-1.5 cursor-pointer mb-1.5">
                  ...
                  <span className="text-[10px] font-semibold text-[var(--sub,#374151)]">5) GCS (Meta + Google)</span>
                </label>
                {cfg('use_gcs', false) && (
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-1.5">
                      <label className="flex items-center gap-1 cursor-pointer">
                        ...
                        <span className="text-[9px] text-[var(--sub,#4b5563)]">Meta</span>
                      </label>
                      <input
                    ...
                    <div className="flex items-center gap-1.5">
                      <label className="flex items-center gap-1 cursor-pointer">
                        ...
                        <span className="text-[9px] text-[var(--sub,#4b5563)]">Google</span>
                      </label>
                      <input
                    ...
                    {cfg('use_google', false) && (
                      <div className="flex items-center gap-1">
                        ...
                        <button className="flex-shrink-0 px-2 h-7 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={() => setChannelManagerOpen(true)}>
                          <Settings2 size={10} />
                        </button>
                      </div>
                    )}
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onMouseDown={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()}
                      />
                      <button className="flex-shrink-0 px-2 h-7 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={handleTestGcs}>Test</button>
                    </div>
                  </div>
                )}
                <div className="flex gap-1 mt-2">
                  <button
                    className="nodrag flex-1 py-1 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)] transition-colors flex items-center justify-center gap-1 disabled:opacity-50"
                    disabled={isRunning}
              ...
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5">
                <label className="flex items-center gap-1.5 cursor-pointer mb-1.5">
                  ...
                  <span className="text-[10px] font-semibold text-[var(--sub,#374151)]">6) Chọn App</span>
                  <button className="ml-auto flex-shrink-0 px-1.5 py-0.5 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={handleTestUnc}>Test UNC</button>
                </label>
                <div className="flex items-center gap-1 mb-1.5">
                  <Search size={10} className="text-[var(--n300,#d1d5db)] flex-shrink-0" />
                  ...
                  <button className="flex-shrink-0 px-1.5 h-7 rounded-md bg-[var(--n100,#f3f4f6)] text-[9px] font-semibold text-[var(--sub,#4b5563)] hover:bg-[var(--n200,#e5e7eb)]" onClick={() => setAppManagerOpen(true)}>Quản lý…</button>
                </div>
                ...
                <div className="max-h-[90px] overflow-y-auto rounded-lg border border-[var(--card-border,#f3f4f6)]">
                  {visibleAppTags.map(tag => (
                    <label key={tag} className="flex items-center gap-1.5 px-1.5 py-0.5 hover:bg-[var(--n50,#f9fafb)] cursor-pointer">
                      <input type="checkbox" checked={selectedApps.includes(tag)} onChange={() => toggleApp(tag)} className="w-3 h-3 accent-red-500" />
                      <span className="text-[9px] text-[var(--sub,#4b5563)] truncate">{apps[tag]?.pinned ? '⭐ ' : ''}{tag}</span>
                    </label>
                  ))}
                  {visibleAppTags.length === 0 && <div className="text-[9px] text-[var(--n400,#9ca3af)] text-center py-2">Không có app</div>}
                </div>
              </div>
            </div>
            ...
            {/* ===== Cột 3: Kết quả Links ===== */}
            <div className="flex flex-col gap-2.5 min-w-0">
              <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5 flex-1 min-h-0">
                <div className={labelCls}>7) Kết quả Links</div>
                {outLinks.unc.length === 0 && outLinks.gcs.length === 0 && (
                  <div className="text-[9px] text-[var(--n400,#9ca3af)] text-center py-4">Chưa có kết quả</div>
                )}
                {outLinks.unc.length > 0 && (
                  <div className="mb-2">
                    <div className="text-[9px] text-[var(--n400,#9ca3af)] mb-1">📁 UNC nội bộ</div>
                    {outLinks.unc.map((l, i) => (
                      <div key={i} className="flex items-center gap-1 mb-1">
                        <span className="text-[9px] text-[var(--sub,#4b5563)] truncate flex-1" title={l}>{basename(l)}</span>
                        <CopyButton text={l} />
                      </div>
                    ))}
                  </div>
                ...
                {outLinks.gcs.length > 0 && (
                  <div>
                    <div className="text-[9px] text-[var(--n400,#9ca3af)] mb-1">📦 GCS</div>
                    {outLinks.gcs.map((l, i) => (
                      <div key={i} className="flex items-center gap-1 mb-1">
                        <span className="text-[9px] text-[var(--sub,#4b5563)] truncate flex-1" title={l}>{l.split('?')[0].split('/').pop()}</span>
                        <CopyButton text={l} />
                      </div>
                    ))}
                  </div>
          ...
          {testMsg && <div className="mt-2 text-[9px] text-center text-[var(--n500,#6b7280)]">{testMsg}</div>}
