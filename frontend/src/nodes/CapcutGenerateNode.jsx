...
import { restartCapcut, fetchSystemStatus } from '../lib/api.js';
  ...
  return (
    <div className="rounded-xl border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2.5 mb-2 nodrag">
      <div className="flex items-center gap-1.5 mb-2">
        <input
          className="flex-1 h-6 px-2 rounded-md border border-[var(--card-border,#e5e7eb)] text-[11px] font-semibold text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-violet-400"
          value={timeline.name}
          onChange={e => patch('name', e.target.value)}
        />
        <button
          className="flex-shrink-0 p-1 rounded-md hover:bg-red-50 text-[var(--n400,#9ca3af)] hover:text-red-500 transition-colors"
          title="Xóa timeline"
          onClick={onRemove}
        >
          <X size={13} />
      ...
      >
        <span className="text-[9px] text-[var(--n400,#9ca3af)] flex-1 truncate">Kéo thả video vào đây</span>
      ...
      <div className="flex items-center gap-1 mb-1.5">
        <input
          type="text"
          className="flex-1 h-5 px-1.5 rounded-md border border-[var(--card-border,#e5e7eb)] text-[9px] text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-violet-400"
      ...
      >
        <span className="text-[9px] text-[var(--n400,#9ca3af)] flex-1 truncate">Kéo thả file nhạc vào đây</span>
      ...
      <div className="mb-2">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[9px] text-[var(--n400,#9ca3af)]">Music</span>
        ...
        />
        <div className="flex justify-between text-[8px] text-[var(--n300,#d1d5db)]">
          <span>-36 dB</span>
          <span>+36 dB</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pt-1.5 border-t border-[var(--card-border,#f3f4f6)]">
        <label className="flex items-center gap-1.5 cursor-pointer">
          ...
          />
          <span className="text-[10px] text-[var(--sub,#4b5563)]">Transition</span>
        </label>
        {timeline.transitions_enabled && (
          <div className="relative">
            <button
              className="flex items-center gap-1 px-1.5 py-0.5 rounded-md border border-[var(--card-border,#e5e7eb)] text-[9px] text-[var(--sub,#4b5563)] hover:border-violet-300 transition-colors"
            ...
            {transOpen && (
              <div className="absolute left-0 top-full mt-1 bg-[var(--card,#fff)] rounded-lg shadow-xl border border-[var(--card-border,#e5e7eb)] py-1 z-[9999] min-w-[130px] max-h-[160px] overflow-y-auto">
                {TRANSITIONS.map(name => (
                  <label key={name} className="flex items-center gap-1.5 px-2 py-1 hover:bg-[var(--n50,#f9fafb)] cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="w-3 h-3 accent-violet-500"
                    />
                    <span className="text-[9px] text-[var(--sub,#374151)]">{name}</span>
                  </label>
                ))}
              </div>
            )}
        ...
        <label className="flex items-center gap-1.5 cursor-pointer ml-auto">
          <input
            ...
            className="w-3 h-3 accent-violet-500"
          />
          <span className="text-[10px] text-[var(--sub,#4b5563)]">Text-template</span>
        </label>
      </div>
    </div>
  );
...
export default function CapcutGenerateNode({ id, data, selected, width }) {
  ...
  return (
    <div className="relative flex flex-col" style={{ width: nodeW, height: '100%', opacity: isActive ? 1 : 0.4 }}>
      ...
      <div className="absolute bottom-full left-0.5 right-0.5 mb-1 flex items-center justify-between">
        <div className="text-[11px] text-[var(--n400,#9ca3af)] font-medium select-none truncate">
          {manifest.name} <span className="text-[var(--n300,#d1d5db)]">#{data.nodeNumber ?? id.slice(-4)}</span>
        </div>
        <button
          className="nodrag flex-shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-[var(--card,#fff)] border border-[var(--card-border,#e5e7eb)] text-[9px] font-semibold text-[var(--n500,#6b7280)] hover:border-violet-300 hover:text-violet-600 transition-colors disabled:opacity-50"
      ...
      >
        ...
        <div className="rounded-2xl bg-[var(--n50,#f9fafb)] p-3 overflow-y-auto flex-1 min-h-0">
          ...
          <div className="flex items-center gap-1.5 mb-3 nodrag">
            <Folder size={11} className="text-[var(--n400,#9ca3af)] flex-shrink-0" />
            <input
              type="text"
              className="flex-1 h-6 px-2 rounded-md border border-[var(--card-border,#e5e7eb)] text-[10px] text-[var(--sub,#374151)] bg-[var(--card,#fff)] focus:outline-none focus:border-violet-400"
