        ...
        <div className="relative w-full rounded-2xl bg-gray-50 px-3 pt-3 pb-2 overflow-y-auto flex-1 min-h-0" style={{ minHeight: DEFAULT_CONTENT_H }}>
          ...
          <div className={`transition-opacity ${cfg('create_folders', false) ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
            <div className="flex items-center gap-1 mb-1.5">
              <input
                type="text"
                className="nodrag flex-1 min-w-0 h-6 px-2 rounded-md border border-gray-200 text-[10px] text-gray-700 bg-white focus:outline-none focus:border-teal-400"
                placeholder="D:/Projects"
                value={cfg('base_path', '')}
                onChange={e => set('base_path', e.target.value)}
              />
              <button
                className="w-6 h-6 flex-shrink-0 flex items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:border-teal-400 hover:text-teal-500 transition-colors nodrag"
                title="Chọn thư mục"
