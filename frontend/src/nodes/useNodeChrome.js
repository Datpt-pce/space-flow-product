...
// Trích từ BaseNode.jsx gốc — logic phi-hình-ảnh dùng chung cho BaseNodeBeta.jsx. Không đổi bất kỳ
// phép tính nào ở đây so với bản gốc.
...
function getIncomingCountByHandle(nodeId, nodes, edges, nodeOutputs) {
  const inEdges = edges.filter(e => e.target === nodeId);
  const counts = {};
  for (const e of inEdges) {
    let n = 0;
    ...
    if (outputs && outputs[e.sourceHandle] !== undefined) {
      ...
      else if (val !== null && val !== undefined) n = 1;
    } else {
      const srcNode = nodes.find(n2 => n2.id === e.source);
      if (srcNode?.type === 'list') n = countListItemsByKind(srcNode.data?.config, e.sourceHandle);
      else if (srcNode?.type === 'text') n = 1;
    }
    counts[e.targetHandle] = (counts[e.targetHandle] || 0) + n;
  }
  return counts;
}
...
export function useNodeChrome(id, data) {
  ...
  const incomingByHandle = getIncomingCountByHandle(id, nodes, edges, nodeOutputs);
  ...
  return {
    ...
    incomingCount, incomingByHandle, inputs, outputs, reachableCount, isActive, isPinned,
