    ...
    <div className="flex flex-col" style={{ width: noteW, height: '100%' }}>
      ...
      <NodeToolbar isVisible={!!selected} position={Position.Top} align="start" offset={8}>
        <div className="flex items-center gap-0.5 bg-[var(--card,#fff)] rounded-2xl shadow-lg border border-[var(--card-border,#e5e7eb)] px-1.5 py-1.5">
          ...
          <div className="relative" ref={colorRef}>
            <button
              className="flex items-center gap-1 px-1.5 py-1 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors nodrag"
              onClick={() => setShowColorPicker(v => !v)}
            >
              <div className="w-4 h-4 rounded-full border border-[var(--n300,#d1d5db)] flex-shrink-0"
                style={{ background: COLOR_MAP[color]?.border || '#FDE68A' }} />
              <ChevronDown size={10} className="text-[var(--n400,#9ca3af)]" />
            </button>
            {showColorPicker && (
              <div className="absolute top-full left-0 mt-1 bg-[var(--card,#fff)] rounded-2xl shadow-xl border border-[var(--card-border,#e5e7eb)] p-2 flex gap-1.5 z-50 nodrag">
          ...
          {/* Format buttons */}
          <button
            className={`p-1.5 rounded-xl transition-colors ${bold ? 'bg-[var(--n100,#f3f4f6)] text-[var(--n800,#1f2937)]' : 'hover:bg-[var(--n100,#f3f4f6)] text-[var(--n500,#6b7280)]'}`}
          ...
          <button
            className={`p-1.5 rounded-xl transition-colors ${italic ? 'bg-[var(--n100,#f3f4f6)] text-[var(--n800,#1f2937)]' : 'hover:bg-[var(--n100,#f3f4f6)] text-[var(--n500,#6b7280)]'}`}
          ...
          <button
            className={`p-1.5 rounded-xl transition-colors ${listMode === 'bullet' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--n800,#1f2937)]' : 'hover:bg-[var(--n100,#f3f4f6)] text-[var(--n500,#6b7280)]'}`}
          ...
          <button
            className={`p-1.5 rounded-xl transition-colors ${listMode === 'numbered' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--n800,#1f2937)]' : 'hover:bg-[var(--n100,#f3f4f6)] text-[var(--n500,#6b7280)]'}`}
          ...
          <button
            className="p-1.5 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors text-[var(--n500,#6b7280)]"
          ...
          <button className="p-1.5 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors text-[var(--n500,#6b7280)]" onClick={() => duplicateNode(id)} title="Duplicate">
            <Copy size={12} />
          </button>
          <button className="p-1.5 rounded-xl hover:bg-red-50 transition-colors text-[var(--n500,#6b7280)] hover:text-red-500" onClick={handleDelete} title="Delete">
            <Trash2 size={12} />
          </button>
        </div>
      </NodeToolbar>
      ...
      >
        ...
        <div className="px-4 pb-3 flex items-center justify-between">
          <span className="text-[10px] text-[var(--n400,#9ca3af)] italic select-none truncate">
            {manifest.name}
          </span>
        </div>
