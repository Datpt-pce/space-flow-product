    ...
    <div className="flex flex-col" style={{ width: nodeW, height: '100%', opacity: isActive ? 1 : 0.4 }}>
      ...
      <NodeToolbar isVisible={!!selected} position={Position.Top} align="start" offset={8}>
        <div className="flex items-center gap-0.5 bg-[var(--card,#fff)] rounded-2xl shadow-lg border border-[var(--card-border,#e5e7eb)] px-1.5 py-1.5">
          ...
          <button className="p-1.5 rounded-xl hover:bg-[var(--n100,#f3f4f6)] transition-colors text-[var(--n500,#6b7280)]" title="Configure" onClick={() => selectNode(id)}>
            <Settings2 size={12} />
          </button>
      ...
      >
        ...
        <div
          ...
          className={`relative w-full rounded-2xl overflow-hidden flex flex-col outline-none transition-colors flex-1 min-h-0 ${
            isFocused ? 'bg-blue-50' : 'bg-[var(--n50,#f9fafb)]'
          }`}
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 pt-2.5 pb-1.5 flex-shrink-0">
            <div className="flex items-center gap-1.5">
              ...
              {mode === 'text' && (
                <button
                  className="nodrag p-0.5 rounded hover:bg-teal-50 text-[var(--n400,#9ca3af)] hover:text-teal-500 transition-colors"
                  onClick={addItem}
            ...
            <div className="flex items-center gap-1 nodrag">
              <div className="flex items-center bg-[var(--card,#fff)] rounded-lg border border-[var(--card-border,#e5e7eb)] p-0.5">
                <button
                  className={`p-1 rounded-md transition-colors ${mode === 'text' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
                ...
                <button
                  className={`p-1 rounded-md transition-colors ${mode === 'table' ? 'bg-[var(--n100,#f3f4f6)] text-[var(--sub,#374151)]' : 'text-[var(--n400,#9ca3af)] hover:text-[var(--sub,#4b5563)]'}`}
              ...
              {isEmpty ? (
                ...
                >
                  {isFocused ? (
                    ...
                    <>
                      <span className="text-[11px] text-[var(--n300,#d1d5db)] text-center">Nhấn + hoặc Ctrl+V</span>
                ...
                <div className="px-2 space-y-0.5">
                  {items.map((item, i) => (
                    <div
                      ...
                      className={`flex items-center gap-1 px-1 py-0.5 rounded hover:bg-[var(--card,#fff)] border-b border-[var(--card-border,#f3f4f6)] last:border-b-0 transition-colors ${
                        dragOverIdx === i && dragIdx !== i ? 'border-t-2 border-t-teal-400 bg-teal-50' : ''
                      } ${dragIdx === i ? 'opacity-40' : ''}`}
                    >
                      <span className="text-[9px] text-[var(--n300,#d1d5db)] w-4 text-right flex-shrink-0 select-none">{i + 1}</span>
                      <GripVertical
                        size={10}
                      ...
                      <input
                        type="text"
                        className="nodrag flex-1 min-w-0 bg-transparent text-[11px] text-[var(--sub,#374151)] focus:outline-none placeholder-[var(--n300,#d1d5db)] py-0.5"
                        value={item}
                ...
                <thead>
                  <tr>
                    {headers.slice(0, TABLE_COL_MAX).map((h, i) => (
                      <th
                        key={i}
                        className="text-left px-2 py-1 bg-[var(--card,#fff)] text-[var(--n500,#6b7280)] font-semibold border-b border-[var(--card-border,#e5e7eb)] truncate"
                        style={{ maxWidth: 72 }}
                      >
                        {h || `Col${i + 1}`}
                      </th>
                    ...
                    {headers.length > TABLE_COL_MAX && (
                      <th className="px-2 py-1 bg-[var(--card,#fff)] text-[var(--n300,#d1d5db)] font-normal text-center">
                        +{headers.length - TABLE_COL_MAX}
                      </th>
                    )}
                  </tr>
                  ...
                  {rows.map((row, ri) => (
                    <tr key={ri} className="border-b border-[var(--card-border,#f3f4f6)] hover:bg-[var(--card,#fff)] transition-colors">
                      {row.slice(0, TABLE_COL_MAX).map((cell, ci) => (
                        <td key={ci} className="px-2 py-1 text-[var(--sub,#4b5563)] truncate" style={{ maxWidth: 72 }}>
                          {cell}
                        </td>
                      ))}
