...
import { fetchResizeUploadV3Catalog, previewResizeUploadV31 } from '../lib/api.js';
...
import ResizeUploadV31Wiring from './ResizeUploadV31Wiring.jsx';
...
export default function ResizeUploadV31Node({ id, data, selected, width }) {
  const config = data.config || {};
  const rows = config.rows || [];
  const rowErrors = Object.fromEntries(rows.map(row => [row.id, resizeDriveRowError('resize-upload-v3-1', config, row)]));
  ...
  const validateOutputFolder = () => {
    if (!testMode && !config.output_folder?.trim()) {
      setMessage('Bắt buộc chọn Output Folder trước khi chạy thật. Mở Cài đặt V3.1 (nút bánh răng) để chọn thư mục.');
      return false;
    }
    return true;
  };
  const inspect = async (mode = null, reviewOnly = false) => {
    ...
    try {
      if (!mode && queuedDriveFolders('resize-upload-v3-1', config).length) throw new Error('Input có link Drive chờ tải. Nhấn chạy node để tải trước khi kiểm tra.');
      const prepared = mode ? await useStore.getState().prepareResizeDriveInputs(id) : config;
      if (!prepared) return;
      const result = await previewResizeUploadV31({ ...prepared, short_video_ack: '', label_timestamp: '' }, connectedInputs());
      setPreview(result);
  ...
  return <div style={{ width: width || 1160, height: '100%', minHeight: 640 }} className="relative flex flex-col text-[var(--text,#111827)]">
    ...
    <div className={`flex flex-col flex-1 min-h-0 rounded-2xl p-4 bg-[var(--card,#fff)] border border-[var(--card-border,#e5e7eb)] shadow-sm ${selected ? 'ring-2 ring-red-400' : ''}`}>
      <div className="flex items-center justify-between gap-3 mb-3">
        <div><div className="flex items-center gap-2 text-sm font-semibold"><Clapperboard size={18} className="text-red-500" />Resize & Upload V3.1</div>
      ...
      <div className="nodrag flex items-center gap-2 mb-3 text-[11px]" role="group" aria-label="Mô hình V3.1">
        ...
        <span>{wiring ? 'Chỉ chạy folder đã nối · Checkbox áp dụng riêng cho từng nhánh' : 'Mỗi folder × các nền tảng đã chọn'}</span>
      </div>
      {wiring ? <ResizeUploadV31Wiring rows={rows} rowErrors={rowErrors} folders={config.wiring_folders || []} upstreamFolders={upstreamFolders} catalog={catalogs.effective}
        onRows={setRows} onFolders={value => { set('wiring_folders', value); set('short_video_ack', ''); }} onTask={setTaskRow}
          ...
          <tbody>{rows.map((row, index) => {
            ...
            return <tr key={row.id} aria-invalid={!!rowError} title={rowError || undefined} className={`align-top border-t border-[var(--card-border,#e5e7eb)] ${rowError ? 'bg-red-500/10 ring-1 ring-inset ring-red-400' : ''}`}>
              ...
              <td className="p-2"><div aria-label={`Thả folder Theme dòng ${index + 1}`} className={`w-44 min-h-28 p-2 rounded-lg border-2 border-dashed flex flex-col gap-2 ${dragRow === row.id ? 'border-red-500 bg-red-50' : 'border-[var(--card-border,#e5e7eb)]'}`} onDragOver={e => { e.preventDefault(); e.stopPropagation(); e.dataTransfer.dropEffect = 'copy'; setDragRow(row.id); }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget)) setDragRow(null); }} onDrop={async e => {
                ...
                <ResizeDriveInput version="v3-1" disabled={running || busy} onImport={paths => addFolders(row, paths)} />
      ...
      <div className="nodrag flex items-center gap-2 mt-3">
        ...
        <button className={button} title="Cài đặt V3.1" onClick={() => setSettingsOpen(true)}><Settings2 size={15} /></button>
      </div>
      <div className="text-[10px] mt-2 text-[var(--sub,#6b7280)]">{testMode ? 'Test: video + thumbnail → Desktop/SpaceFlow-Resize-Upload-V3.1-Test. Drive và Asana chưa chạy.' : 'Đích Drive và thumbnail lấy theo App / nền tảng trong thư viện của bạn.'}</div>
      {displayProgress && <div className="mt-2"><div role="progressbar" aria-label="Tiến độ Resize V3.1" aria-valuemin={0} aria-valuemax={100} aria-valuenow={displayProgress.percent} className="h-1.5 rounded bg-[var(--n100,#f3f4f6)] overflow-hidden"><div className={`h-full rounded bg-red-500 ${displayProgress.percent == null ? 'animate-pulse' : ''}`} style={{ width: displayProgress.percent == null ? '100%' : `${displayProgress.percent}%` }} /></div><div role="status" className="text-[10px] mt-1 truncate">{displayProgress.message}</div></div>}
    ...
    {settingsOpen && createPortal(<Modal title="Cài đặt Resize & Upload V3.1" width={520} onClose={() => setSettingsOpen(false)}><div className="flex flex-col gap-3 text-[11px] text-[var(--text,#111827)]">
    ...
    {previewOpen && preview && createPortal(<Modal title="Kiểm tra Theme / Ngôn ngữ / Video" width={780} onClose={() => setPreviewOpen(false)}><div className="flex flex-col gap-3 text-[12px] text-[var(--text,#111827)]">
      <p><strong>{preview.combinations ?? 0} tổ hợp folder/nền tảng</strong> · {wiring ? 'Theo dây đã nối' : 'Theo bảng V3'}</p>
      ...
      <ResizeInputPreview nodeId={id} paused={running} preview={preview} config={config} set={set} inspect={previewResizeUploadV31} inputs={connectedInputs} onPreview={setPreview} onBusy={setBusy} />
