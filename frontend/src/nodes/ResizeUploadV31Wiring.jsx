...
import { basename, inputCls, resolveDropPaths, MODES } from './resizeUploadShared.jsx';
...
export default function ResizeUploadV31Wiring({ rows, folders, upstreamFolders, catalog, onRows, onFolders, onTask, onMessage, pickFolder, makeRow, output, disabled, rowErrors = {} }) {
  ...
  const add = paths => { onFolders([...new Set([...folders, ...paths])]); onMessage(''); };
  const addManual = () => { const paths = manual.split(/\r?\n/).map(value => value.trim().replace(/^"|"$/g, '')).filter(Boolean); if (paths.length) { add(paths); setManual(''); } };
  ...
  return <div className="nodrag nowheel flex-1 min-h-0 flex flex-col text-[11px]" onKeyDown={event => { event.stopPropagation(); if (event.key === 'Escape') { setPending(null); setPointer(null); } }}>
    <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
      <p aria-label="Tổng hợp nối dây"><strong>{allFolders.length} folder · {selectedRows.length} nhánh · {count} tổ hợp folder/nền tảng</strong>{unused > 0 && <span className="text-amber-600"> · {unused} folder chưa nối vào nhánh đang chọn, sẽ bỏ qua</span>}</p>
    ...
    <div className="text-[10px] text-[var(--sub,#6b7280)] mb-2">Kéo từ chấm tròn sang cổng đích, hoặc bấm lần lượt hai cổng. Bấm dây để gỡ. Có thể chọn nhiều folder rồi nối cùng lúc.</div>
    <div className="overflow-auto min-h-[210px] flex-1 border border-[var(--card-border,#e5e7eb)] rounded-xl">
      <fieldset disabled={disabled} className="min-w-[990px] min-h-full border-0 p-4" onPointerUp={finish} onPointerCancel={() => { origin.current = null; setPending(null); setPointer(null); }}>
        <div className="grid grid-cols-[240px_1fr_210px] gap-x-20 mb-3 font-semibold text-[var(--sub,#6b7280)]"><span>1. FOLDER INPUT</span><span>2. NHÁNH ASANA / CẤU HÌNH</span><span>3. NỀN TẢNG</span></div>
        <div ref={board} className="relative grid grid-cols-[240px_1fr_210px] gap-x-20 items-start" onPointerMove={event => {
          ...
          <div className="flex flex-col gap-2">
            <div className={`${card} border-dashed`} data-wire-card aria-label="Thả folder vào sơ đồ" onDragOver={event => { event.preventDefault(); event.stopPropagation(); }} onDrop={async event => {
              event.preventDefault(); event.stopPropagation(); if (disabled) return;
              try { const paths = await resolveDropPaths(event); if (!paths.length) throw new Error('Không lấy được đường dẫn folder. Dùng Chọn folder hoặc dán đường dẫn.'); add(paths); }
              catch (error) { onMessage(error.message); }
            }}>
              <div className="flex gap-2 items-center mb-2"><FolderTree size={16} />Kéo thả nhiều folder Theme</div>
              <ResizeDriveInput version="v3-1" disabled={disabled} onImport={add} />
              <textarea aria-label="Folder trong sơ đồ" className={`${inputCls} h-14 resize-none`} placeholder="Mỗi dòng một đường dẫn Theme…" value={manual} onChange={e => setManual(e.target.value)} />
              <div className="flex gap-2 mt-2"><button className={button} onClick={addManual}>Thêm path</button><button className={button} onClick={async () => { const path = await pickFolder(); if (path) add([path]); }}>Chọn folder…</button></div>
            </div>
            ...
            {allFolders.map((path, i) => <div key={path} className={`${card} flex items-center gap-2`} data-wire-card>
              ...
              <span className="truncate flex-1" title={path}>{basename(path)}</span><ResizeOpenFolderButton path={path} />
              {!upstreamFolders.includes(path) && <button aria-label={`Xóa folder ${basename(path)}`} onClick={() => { onFolders(allFolders.filter(p => p !== path && !upstreamFolders.includes(p))); onRows(rows.map(row => ({ ...row, input_folders: row.input_folders.filter(p => p !== path) }))); setSelection(selection.filter(p => p !== path)); }}><X size={12} /></button>}
          ...
          <div className="flex flex-col gap-4">
            {rows.map((row, i) => <div key={row.id} aria-invalid={!!(rowErrors[row.id] || output?.rows?.[row.id]?.error)} className={card} style={{ borderTop: `3px solid ${colors[i % colors.length]}`, opacity: row.selected ? 1 : 0.55, ...(rowErrors[row.id] || output?.rows?.[row.id]?.error ? { background: 'color-mix(in srgb, #ef4444 10%, var(--card, #fff))', outline: '1px solid #f87171' } : {}) }} data-wire-card>
              ...
              <div className="mt-2 text-[10px] text-[var(--sub,#6b7280)]">{row.input_folders.length} folder → {row.platforms.length} nền tảng · {row.input_folders.length * row.platforms.length} tổ hợp</div>
