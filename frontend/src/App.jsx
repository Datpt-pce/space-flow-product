...
import ResizeDrivePreparation from './nodes/ResizeDrivePreparation.jsx';
...
const ReviewWorkspace = lazy(() => import('./contributions/ReviewWorkspace.jsx'));

export default function App() {
  const [isReviewOpen, setReviewOpen] = useState(() => window.location.hash === '#review');
  useEffect(() => {
    const changed = () => setReviewOpen(window.location.hash === '#review');
  ...
  const reconnectRuns = useStore(s => s.reconnectRuns);

  useEffect(() => { reconnectRuns(); }, [currentUser?.id, reconnectRuns]);
  ...
  if (isReviewOpen) return <><Suspense fallback={null}><ReviewWorkspace /></Suspense><SettingsModal /></>;
