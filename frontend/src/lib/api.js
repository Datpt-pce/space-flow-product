...
export async function uploadVideoMedia(file) {
  ...
  body.append('originalName', file.name); // UTF-8 text field, independent of multipart filename encoding.
...
export async function videoWorkspaceRequest(path = '', body, method) {
  const response = await apiFetch(`/api/video-projects${path}`, body === undefined ? { method: method || 'GET' } : {
...
export async function retryVideoAssetPreview(id) {
  const response = await apiFetch(`/api/video-assets/${encodeURIComponent(id)}/preview/retry`, { method: 'POST' });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Không tạo lại được preview');
  return data;
}
...
export { fetchWorkflows, createWorkflow, updateWorkflow, loadWorkflow, deleteWorkflowFromLibrary } from './workflowApi';
...
export async function fetchReportingConnections() {
  const res = await apiFetch('/api/google-oauth/reporting/connections');
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Không tải được kết nối Google');
  return data;
}
export async function startReportingConnection(name, scope) {
  const res = await apiFetch('/api/google-oauth/reporting/connect', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, scope }) });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Không bắt đầu được kết nối Google');
  return data;
}

async function resizeUploadV3Request(action, method = 'GET', body) {
  const res = await apiFetch(`/api/resize-upload-v3/${action}`, {
    method, headers: { 'Content-Type': 'application/json' },
    ...(body === undefined ? {} : { body: JSON.stringify(body) }),
  });
  const result = await res.json();
  if (!res.ok) throw new Error(result.error || 'Resize & Upload V3: yêu cầu thất bại');
  return result;
}
export const fetchResizeUploadV3Catalog = () => resizeUploadV3Request('catalog');
export const saveResizeUploadV3Catalog = (scope, data) => resizeUploadV3Request('catalog', 'POST', { scope, data });
export const deleteResizeUploadV3Catalog = () => resizeUploadV3Request('catalog', 'DELETE');
export const previewResizeUploadV3 = (config, inputs) => resizeUploadV3Request('preview', 'POST', { config, inputs });
export async function resizeInputAction(version, config, action) {
  if (!['resize-upload-v3', 'resize-upload-v3-1', 'resize-upload-v3-2'].includes(version)) throw new Error('Phiên bản Resize không hợp lệ');
  const response = await apiFetch(`/api/${version}/input-action`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ config, ...action }) });
  const result = await response.json();
  if (!response.ok) throw new Error(result.error || 'Không thay đổi được Input');
  return result;
}
export async function importResizeDriveFolder(version, url, onProgress, { importId, checkOnly, onSession, signal } = {}) {
  if (!['v3', 'v3-1', 'v3-2'].includes(version)) throw new Error('Phiên bản Resize không hợp lệ');
  const response = apiFetch(`/api/resize-upload-${version}/import-drive`, {
    signal,
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ url, importId, checkOnly, config: { rows: [] } }),
  }).then(async res => {
    if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Không tải được Drive'); }
    return res;
  });
  let result, failure;
  await readEventStream(response, (event, data) => {
    if (event === 'session') onSession?.(data.importId);
    if (event === 'progress') onProgress(data.message);
    if (event === 'complete') result = data;
    if (event === 'error') failure = data.error;
  });
  if (failure || (!result?.status && !result?.folder)) throw new Error(failure || 'Kết nối bị ngắt trước khi tải xong; hãy thử lại');
  return result;
}
export async function previewResizeUploadV31(config, inputs) {
  const res = await apiFetch('/api/resize-upload-v3-1/preview', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ config, inputs }) });
  const data = await res.json();
  if (!res.ok || data.error) throw new Error(data.error || 'Không kiểm tra được Input V3.1');
  return data;
}
async function resizeUploadV32Request(action, body) {
  const res = await apiFetch(`/api/resize-upload-v3-2/${action}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  const data = await res.json();
  if (!res.ok || data.error) throw new Error(data.error || 'V3.2 không xử lý được yêu cầu');
  return data;
}
export const recognizeResizeUploadV32 = (config, inputs) => resizeUploadV32Request('recognize', { config, inputs });
export const readResizeUploadV32Labels = config => resizeUploadV32Request('labels', { config });
export const fetchResizeUploadV32Tabs = config => resizeUploadV32Request('tabs', { config });
export const previewResizeUploadV32 = (config, inputs) => resizeUploadV32Request('preview', { config, inputs });
export const reportResizeUploadV32 = (config, groups, preview) => resizeUploadV32Request('report', { config, groups, preview });
export function runResizeUploadV3(config, inputs, mode, onEvent) {
  const response = apiFetch('/api/resize-upload-v3/run', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ config, inputs, mode }),
  }).then(async res => {
    if (!res.ok) {
      const body = await res.json().catch(() => null);
      throw new Error(body?.error || `Resize & Upload V3: yêu cầu thất bại (HTTP ${res.status})`);
    }
    if (!res.headers.get('content-type')?.includes('text/event-stream')) {
      throw new Error('Agent không trả về luồng tiến độ Resize V3. Kiểm tra phiên bản Agent trong Cài đặt → Agent.');
    }
    return res;
  });
      ...
      function processBuffer() {
        ...
        while ((sepIndex = buffer.indexOf('\n\n')) !== -1) {
          ...
          if (eventType && dataLine != null) {
            ...
            // A consumer may reject a valid error event. Only malformed JSON is ignored.
