...
async function drawSample(sample, ctx, transform, resolution, compositeOperation, colorGradeFilter, crop, sourceSize, clip, scratch, previewScale) {
  ...
  if ((clip.mask && clip.mask.enabled !== false) || (clip.effects || []).some(effect => effect.type === 'chromaKey' && effect.enabled)) {
    const factor = Math.min(previewScale, 1280 / Math.max(destWidth, destHeight));
...
export class CanvasEngine {
  ...
  drawActiveClips(ctx, entries, playheadMs, resolution, previewScale = 1) {
    if (this.disposed) return Promise.resolve();
    this.stats.requested++;
    if (this.pendingDraw) this.stats.coalesced++;
    this.pendingDraw = { ctx, entries, playheadMs, resolution, previewScale };
  ...
  async drainDraws() {
    while (this.pendingDraw && !this.disposed) {
      ...
      this.pendingDraw = null;
      await this.drawFrame(request.ctx, request.entries, request.playheadMs, request.resolution, request.previewScale);
    }
  }

  async drawFrame(ctx, entries, playheadMs, resolution, previewScale = 1) {
    const token = ++this.renderToken;
    const width = Math.max(2, Math.round(resolution.width * previewScale));
    const height = Math.max(2, Math.round(resolution.height * previewScale));
    if (ctx.canvas.width !== width) ctx.canvas.width = width;
    if (ctx.canvas.height !== height) ctx.canvas.height = height;

    const started = performance.now();
    // Independent clip cursors can decode concurrently. allSettled preserves
    // layer order and lets a failed source leave the other layers visible.
    const decoded = await Promise.allSettled(entries.map(async ({ clip, proxyUrl, kind, transition, sampleTimeMs = playheadMs }) => {
      ...
      if (this.disposed || token !== this.renderToken) { sample?.close(); return null; }
      ...
      return { sample, transform, trackEntry, clip, transition, sampleTimeMs };
    }));
    const draws = decoded.filter(result => result.status === 'fulfilled' && result.value).map(result => result.value);
    ...
    const paint = async (draw, target, blend = blendModeFor(draw.clip)) => {
      target.save(); target.scale(width / resolution.width, height / resolution.height);
      try {
        await drawSample(draw.sample, target, draw.transform, resolution, blend,
          previewFilterFor(draw.clip, draw.sampleTimeMs), normalizedCropFor(draw.clip), draw.clip.sourceSize, draw.clip, this.scratch, previewScale);
