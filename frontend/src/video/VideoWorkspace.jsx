...
import './video.css';
import BatchLab from './batch/BatchLab';
import CreativeAssistant from './assistant/CreativeAssistant';
import BatchReturn from './batch/BatchReturn';
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <button
        ...
        title="Thư viện timeline"
        aria-label="Thư viện timeline"
...
export default function VideoWorkspace() {
  const assistantMode = new URLSearchParams(window.location.search).has('assistant');
  const labMode = new URLSearchParams(window.location.search).has('lab');
  const labMini = new URLSearchParams(window.location.search).get('labMini') === '1';
  ...
  const [previewScale, setPreviewScale] = useState(() => {
    const value = Number(localStorage.getItem('video-preview-scale'));
    return [1, .5, .25].includes(value) ? value : 1;
  });
  const changePreviewScale = value => { setPreviewScale(value); localStorage.setItem('video-preview-scale', String(value)); };
  ...
  useEffect(() => {
    if (currentUser && !labMode && !assistantMode) loadOrCreateProject();
  }, [currentUser, loadOrCreateProject, labMode, assistantMode]);

  if (!authChecked) return null;
  if (!currentUser) return <LoginGate />;
  if (assistantMode) return <CreativeAssistant />;
  if (labMode) return <BatchLab />;
  ...
  return (
    ...
    >
      <header style={{ gridArea: 'header' }} className="flex items-center gap-3 px-4 h-11 border-b border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] text-xs">
        ...
        {!labMini && <ProjectTimelines mode="project" />}
        {!labMini && <a href="/video?lab" className="shrink-0 rounded border border-[var(--card-border)] px-2 py-1">Batch Creative Lab</a>}
        {!labMini && <a href="/video?assistant" className="shrink-0 rounded border border-[var(--card-border)] px-2 py-1">Creative Assistant</a>}
        <BatchReturn />
      ...
      <main style={{ gridArea: 'preview' }} aria-label="Xem trước" className="min-w-0 min-h-0 flex flex-col border-l border-[var(--card-border)]">
        ...
        <div className="flex-1 min-h-0">
          <Player zoomMode={zoomMode} previewVolume={previewVolume} previewScale={previewScale} />
        </div>
        <TransportBar zoomMode={zoomMode} setZoomMode={setZoomMode} previewVolume={previewVolume} setPreviewVolume={setPreviewVolume} previewScale={previewScale} setPreviewScale={changePreviewScale} />
      </main>
      ...
      <div style={{ gridArea: 'timeline' }} className="relative min-h-0 flex flex-col border-t border-[var(--card-border,#e5e7eb)] overflow-hidden">
        {timelinePanel.collapsed ? (
          ...
          <>
            ...
            <div className="flex shrink-0 items-center min-w-0 pr-8"><ProjectTimelines mode="timeline" />{project && <ProjectSwitcher currentProjectId={project.id} />}</div>
            <Timeline />
          </>
        )}
      </div>
