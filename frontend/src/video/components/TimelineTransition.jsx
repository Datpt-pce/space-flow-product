...
export default function TimelineTransition({ track, fromClip, toClip, transition, timeToX, pxToMs, msToPx, onToggle }) {
  ...
  return <>
    <div className="absolute top-4 h-10 z-20 rounded border border-[var(--accent)] bg-[var(--accent-tint)]" style={{ left: timeToX(toClip.timelineInMs) - Math.max(16, msToPx(duration)) / 2, width: Math.max(16, msToPx(duration)) }}
      onMouseDown={e => e.stopPropagation()} onContextMenu={e => { e.preventDefault(); e.stopPropagation(); setMenu({ x: Math.min(e.clientX, window.innerWidth - 200), y: Math.min(e.clientY, window.innerHeight - Math.min(window.innerHeight - 16, transitionCatalog.length * 36 + 16)) }); }}>
    ...
    {menu && <div ref={menuRef} role="menu" aria-label="Loại transition" className="fixed z-50 bg-[var(--card)] border border-[var(--card-border)] rounded-lg shadow-xl p-2 text-xs" style={{ left: Math.max(8, menu.x), top: Math.max(8, menu.y), maxHeight: 'calc(100vh - 16px)', overflowY: 'auto' }}>
      {transitionCatalog.map(({ type, name }) => <button key={type} type="button" role="menuitemradio" aria-checked={(transition?.type || 'crossfade') === type} disabled={track.locked} className="flex items-center gap-2 w-full px-4 py-2 text-left hover:bg-[var(--accent-tint)]" onClick={() => {
        ...
        setMenu(null);
      }}><Check aria-hidden="true" size={12} className={(transition?.type || 'crossfade') === type ? 'text-[var(--accent)]' : 'invisible'} />{name}</button>)}
    </div>}
  </>;
}
