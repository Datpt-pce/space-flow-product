...
import { Film, Music, Image as ImageIcon, RefreshCw, AlertCircle, Loader2, Mic, Square, Search, Layers, X, Trash2, LayoutGrid, List, FolderOpen } from 'lucide-react';
...
import AssetPreviewStatus from './AssetPreviewStatus.jsx';
...
export default function MediaBin() {
  ...
  // View preferences are browser-local; both layouts share the same asset order and selection.
  const [viewMode, setViewMode] = useState(() => localStorage.getItem('video-media-view-mode') === 'list' ? 'list' : 'grid');
  ...
  // Every status is selectable for removal. Only ready assets can be dragged or
  // used to create timelines; those action gates remain separate below.
  ...
  // Roving focus follows visible card order, including offline/error media.
  function moveFocusFrom(currentId, step) {
    const ids = filteredAssets.map((a) => a.id);
    const next = ids.indexOf(currentId) + step;
    if (ids[next]) gridRef.current?.querySelector(`[data-asset-id="${ids[next]}"]`)?.focus();
  }
  ...
  function handleGridKeyDown(e) {
    ...
    if (e.key === 'Delete' || e.key === 'Backspace') {
      e.preventDefault(); e.stopPropagation(); handleDeleteSelected(); return;
    }
    if (isMod(e) && e.key.toLowerCase() === 'a') {
      e.preventDefault();
      setAssetSelection(filteredAssets.map((a) => a.id));
      return;
    }
    ...
    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight' || e.key === 'ArrowUp' || e.key === 'ArrowDown') {
      ...
      const card = e.target.closest('[data-asset-id]');
        const columns = viewMode === 'list' ? 1 : Math.max(1, getComputedStyle(card.parentElement).gridTemplateColumns.split(' ').length);
  ...
  // one Tab stop into the grid whenever at least one asset exists — e.g. right after a search
  // query filters out the previously-focused card, or on first render before anything's been focused.
  const effectiveFocusedId = filteredAssets.some((a) => a.id === focusedAssetId) ? focusedAssetId : filteredAssets[0]?.id;
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--card,#fff)]"
      ...
      <div role="group" aria-label="Chế độ xem Media Bin" className="flex items-center gap-1 px-4 py-1.5">
        {[['grid', LayoutGrid, 'Grid'], ['list', List, 'List']].map(([value, Icon, label]) => <button key={value} type="button"
          aria-label={`Media Bin ${label}`} aria-pressed={viewMode === value}
          onClick={() => { setViewMode(value); localStorage.setItem('video-media-view-mode', value); }}
          className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${viewMode === value ? 'bg-[var(--accent-tint)] text-[var(--text)]' : 'text-[var(--n600)] hover:bg-[var(--n100)]'}`}>
          <Icon size={13} />{label}
        </button>)}
      </div>
      {viewMode === 'grid' && <label className="flex items-center gap-2 px-4 py-1 text-[10px] text-[var(--n600)]">Kích thước asset
      ...
      <div className="px-4 py-1.5 border-b border-[var(--card-border,#f3f4f6)] text-xs">
        ...
        {showPathImport && (
          <form className="flex flex-col gap-2 py-2" onSubmit={async (e) => {
            ...
            <div className="flex gap-2"><input id="video-import-path" value={sourcePathInput} onChange={e => setSourcePathInput(e.target.value)} autoFocus className="min-w-0 w-full rounded-lg border border-[var(--card-border,#e5e7eb)] bg-[var(--card,#fff)] p-2 focus-visible:ring-2 focus-visible:ring-[var(--accent,#7C5CFA)]" /><button type="button" aria-label="Chọn file cho đường dẫn Media Bin" title="Duyệt file" disabled={!!importingPath || isRecording} onClick={async () => { const selected = await useStore.getState().pickFile('media'); if (selected) setSourcePathInput(selected); }} className="rounded-lg border border-[var(--card-border)] px-2"><FolderOpen size={16}/></button></div>
      ...
      <div
        ...
        // empty state (nothing for roving tabindex to land on below) — normally the one
        // card at `effectiveFocusedId` is the grid's single Tab stop instead (see its own
        // comment), matching the WAI-ARIA APG composite-widget pattern rather than a plain focusable
        // container.
        tabIndex={filteredAssets.length === 0 ? 0 : -1}
      ...
      >
        ...
        <div data-media-view={viewMode} className={viewMode === 'list' ? 'flex flex-col gap-1.5' : 'grid gap-3'} style={viewMode === 'grid' ? { gridTemplateColumns: `repeat(auto-fill, minmax(min(100%, ${cardSize}px), 1fr))` } : undefined}>
          {filteredAssets.map((asset) => {
            ...
            const statusBadge = <span className="inline-flex items-center gap-1" style={viewMode === 'grid' ? { color: STATUS_TOKEN[asset.status] } : undefined}>
            ...
            return (
              <div
                ...
                tabIndex={asset.id === effectiveFocusedId ? 0 : -1}
                ...
                className={`min-w-0 border overflow-hidden flex ${viewMode === 'list' ? 'flex-row items-center rounded-lg' : 'flex-col rounded-xl'} outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent,#7C5CFA)] focus-visible:ring-offset-1 ${draggable ? 'cursor-grab' : 'cursor-pointer'} ${isSelected ? 'border-[var(--accent,#7C5CFA)] ring-2 ring-[var(--accent,#7C5CFA)]' : 'border-[var(--card-border,#e5e7eb)]'}`}
              >
                ...
                <div className={`relative overflow-hidden bg-[var(--n100,#f3f4f6)] flex items-center justify-center ${viewMode === 'list' ? 'w-16 h-12 shrink-0 ml-2 rounded' : 'aspect-video'}`}>
                  ...
                  {viewMode === 'grid' && <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-1 px-1.5 py-1 text-[10px] font-medium text-white bg-black/80">
                    <span>{formatDuration(asset.durationMs)}</span>
                    {statusBadge}
                  </div>}
                </div>
                <div className={`min-w-0 p-2 flex flex-col gap-1 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                  <p className="text-xs font-medium text-[var(--n600,#4b5563)] truncate" title={asset.sourcePath}>
                    {asset.sourcePath.split(/[\\/]/).pop()}
                  </p>
                  {viewMode === 'list' && <p className="text-[10px] text-[var(--n500)] truncate" title={asset.sourcePath}>{asset.sourcePath}</p>}
                  <AssetPreviewStatus asset={asset} allowRetry />
                  <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 text-[11px] text-[var(--n600,#4b5563)]">
                    {viewMode === 'list' && <>{statusBadge}<span>{formatDuration(asset.durationMs)}</span></>}
