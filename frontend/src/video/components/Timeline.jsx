...
export default function Timeline() {
  ...
  function handleDropTrack(track, e) {
    ...
    if (e.dataTransfer.types.includes('application/x-video-clip')) {
      ...
      // A pasted clip is often the only clip on its row. Moving that clip into
      // the lane must still merge it; row reordering has an explicit drop area.
      const reorderDrop = atTopEdge || !!e.target.closest('[data-track-header]');
      if (wholeTracks && reorderDrop && !gesture.duplicate && !sources.some(t => t.id === track.id)) {
        try {
