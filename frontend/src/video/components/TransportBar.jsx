...
export default function TransportBar({ zoomMode, setZoomMode, previewVolume, setPreviewVolume, previewScale, setPreviewScale }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 px-2 py-1 shrink-0 border-t border-[var(--card-border,#f3f4f6)] bg-[var(--card,#fff)]">
      ...
      <AspectRatioControl />
      <label className="flex items-center gap-1 text-xs text-[var(--n600)]">
        Preview
        <select aria-label="Chất lượng preview" title="Giảm độ phân giải xem trước để phát nhẹ hơn; không đổi chất lượng export"
          value={previewScale} onChange={e => setPreviewScale(Number(e.target.value))}
          className="h-7 rounded border border-[var(--card-border)] bg-[var(--card)] px-1 focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
          <option value={1}>Đầy đủ</option><option value={.5}>1/2</option><option value={.25}>1/4</option>
        </select>
      </label>
    </div>
  );
