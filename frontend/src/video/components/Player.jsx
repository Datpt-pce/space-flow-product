...
import CaptionGraphic from './CaptionGraphic';
...
function CanvasPlayer({ projectState, assets, playheadMs, mediaRef, onSelectTopClip, zoomMode, previewScale }) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => setSpace({ width: Math.max(0, host.clientWidth - 32), height: Math.max(0, host.clientHeight - 32) });
    measure();
  ...
  useEffect(() => {
    ...
    engine.drawActiveClips(ctx, entries, playheadMs, projectState.resolution, previewScale).catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectState, assets, playheadMs, previewScale]);

  const resolution = projectState?.resolution || { width: 1920, height: 1080 };
  const displayScale = zoomMode === '100' ? 1 : Math.min(1, space.width / resolution.width, space.height / resolution.height);
  ...
  return (
    <div ref={hostRef} className="relative w-full h-full bg-[var(--canvas)] flex items-center justify-center overflow-hidden p-4">
      {activeVideoClips.length === 0 ? (
        <p className="text-xs text-[var(--n600)]">Không có clip nào tại vị trí playhead</p>
      ) : !hasResolvedAsset ? (
        <p className="text-xs text-[var(--n600)]">{activeVideoClips.some(clip => assets.some(a => a.id === clip.assetId && ['queued', 'running'].includes(a.previewStatus))) ? 'Đang tạo preview · bạn vẫn có thể sắp xếp clip và render từ file gốc' : 'Asset chưa sẵn sàng (đang xử lý hoặc offline)'}</p>
      ) : (
        <canvas
          ref={(el) => { canvasRef.current = el; mediaRef?.(el); }}
          width={Math.max(2, Math.round(resolution.width * previewScale))} height={Math.max(2, Math.round(resolution.height * previewScale))}
          className={zoomMode === '100' ? 'cursor-pointer shrink-0' : 'max-w-full max-h-full cursor-pointer'}
          style={{ width: resolution.width * displayScale, height: resolution.height * displayScale }}
...
function CaptionOverlay({ projectState, playheadMs, mediaRect }) {
  ...
  return (
    <div className="absolute pointer-events-none" style={mediaRect ? { left: mediaRect.left, top: mediaRect.top, width: mediaRect.width, height: mediaRect.height } : { inset: 0 }}>
      {activeCues.map(cue => <CaptionGraphic key={cue.id} text={cue.text} resolution={projectState.resolution}/>)}
    </div>
  );
}
...
export default function Player({ zoomMode = 'fit', previewVolume = 1, previewScale = 1 }) {
  ...
  const props = { projectState, assets, playheadMs, isPlaying, mediaRef, onSelectTopClip: handleSelectTopClip, zoomMode, previewVolume, previewScale };
  return (
    <div
      ref={containerRef}
      tabIndex={0}
