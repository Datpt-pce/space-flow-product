...
import {
  ...
  fetchVideoCapability, promoteRenderJobToAsset, fetchVideoProjectStateAtSeq, videoWorkspaceRequest,
} from '../lib/api.js';
...
import { createBatchSlice } from './batch/batchSlice';
import { createAssistantSlice } from './assistant/assistantSlice';
...
export const useVideoStore = create((set, get) => ({
  ...
  fetchAssets: async ({ silent = false } = {}) => {
    const versionAtStart = get().assetsVersion;
    if (!silent) set({ loading: true, error: null });
    try {
      const assets = await fetchVideoAssets();
      if (get().assetsVersion !== versionAtStart) { if (!silent) set({ loading: false }); return; }
      set({ assets, ...(!silent ? { loading: false } : {}), assetsVersion: versionAtStart + 1 });
    } catch (err) {
      if (silent) return;
  ...
  // meant to do either).
  workspaceGroupsVersion: 0,
  deleteProject: async (id, { workspace = false } = {}) => {
    if (get().pendingCommands.length) throw new Error('Chờ lưu các chỉnh sửa trước khi xoá.');
    const result = workspace ? await videoWorkspaceRequest(`/${id}/workspace-group`, undefined, 'DELETE') : await deleteVideoProject(id);
    const deletedIds = workspace ? result.deletedTimelineIds : [id];
    set(s => ({ workspaceGroupsVersion: s.workspaceGroupsVersion + 1 }));
    if (!deletedIds.includes(get().project?.id)) return;
    set({ project: null, commandStack: null, projectState: null, projectError: null });
    const params = new URLSearchParams(window.location.search);
    if (deletedIds.includes(params.get('projectId'))) {
