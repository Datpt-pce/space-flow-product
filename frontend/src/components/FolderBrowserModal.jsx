...
import { useDialogFocus } from '../video/useDialogFocus';
...
export default function FolderBrowserModal() {
  ...
  const dialogRef = useDialogFocus(cancel, isOpen);
  ...
  return createPortal(
    ...
    >
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={mode === 'file' ? 'Chọn file' : 'Chọn thư mục'} className="bg-[var(--card,#fff)] rounded-2xl shadow-2xl border border-[var(--card-border,#e5e7eb)] flex flex-col overflow-hidden" style={{ width: 'min(900px, 96vw)', height: 'min(600px, 92dvh)' }}>
        {/* Header */}
