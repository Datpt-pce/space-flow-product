...
import { Plus, Play, Hand, Scissors, Square, MessageSquare, Undo2, Redo2, Save, FolderOpen, Activity, Settings, Library, Share2, Network, Table2, Download, Cloud, Link2, ShieldCheck } from 'lucide-react';
...
export default function Toolbar() {
  ...
  const activeHistoryNode = useStore(s => s.nodes.some(node => node.id === s.activeNodeHistoryId) ? s.activeNodeHistoryId : null);
  const nodeHistory = useStore(s => s.nodeHistories[s.activeNodeHistoryId]);
  const historyBusy = useStore(s => s.nodeHistoryBusy || s.nodeStatuses[s.activeNodeHistoryId] === 'running');
  ...
  const undo = () => { const state = useStore.getState(); return activeHistoryNode ? state.undoNode(activeHistoryNode) : state.undo(); };
  const redo = () => { const state = useStore.getState(); return activeHistoryNode ? state.redoNode(activeHistoryNode) : state.redo(); };
  ...
  const canUndo = activeHistoryNode ? !historyBusy && !!nodeHistory?.undo.length : _undoStack.length > 0;
  const canRedo = activeHistoryNode ? !historyBusy && !!nodeHistory?.redo.length : _redoStack.length > 0;
  ...
  if (activeModule === 'sheet') {
    ...
    return (
      <div className="sf-toolbar absolute left-3 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-1 bg-[var(--card,#fff)] border border-[var(--card-border,#e5e7eb)] rounded-2xl shadow-md px-1.5 py-2">
        {moduleSwitcher}
        ...
        {iconBtn(<ShieldCheck size={15} />, 'Đóng góp & Control Center', () => { window.location.hash = 'review'; })}
  ...
  return (
    <div className="sf-toolbar absolute left-3 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-1 bg-[var(--card,#fff)] border border-[var(--card-border,#e5e7eb)] rounded-2xl shadow-md px-1.5 py-2">
      ...
      {isRunning && <button title="STOP tất cả tác vụ" aria-label="STOP tất cả tác vụ"
        onClick={() => stopRuns()} className="w-9 h-9 rounded-xl bg-red-600 text-white text-[9px] font-bold">STOP</button>}
