...
export function CredentialField({ field, value, onChange }) {
  ...
  useEffect(() => {
    fetchCredentials().then(items => setList(field.credentialType ? items.filter(c => c.type === field.credentialType) : items)).catch(() => setList([]));
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1.5">
        <select aria-label={field.label} className={inputClass + ' flex-1'} value={value ?? ''} onChange={e => onChange(e.target.value)}>
