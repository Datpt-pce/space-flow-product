...
import { fetchSystemStatus, updateDependencies, fetchAutoUpdateConfig, updateAutoUpdateConfig, fetchLatestVersion, fetchCredentials, saveCredential, deleteCredential, fetchLocalServices, saveLocalService, deleteLocalService, fetchUsers, updateUserRole, updateUserStatus, fetchUserPermissions, saveUserPermissions, fetchUsageStats, fetchNodes, fetchMyAgents, createAgent, deleteAgent, fetchLocalNodes, deleteLocalNodeDraft, setInstalledNodeApprovedPaths, acknowledgeInstalledRevocation, fetchRegistryQueue, fetchRegistrySubmissionDetail, approveRegistrySubmission, requestRegistryChanges, deprecateRegistryVersion, revokeRegistryVersion, rollbackRegistryVersion, fetchPublicRegistry, installPublishedNode } from '../lib/api.js';
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[var(--card-border,#f3f4f6)]">
            <h2 className="text-base font-semibold text-[var(--text,#111827)]">
              {tab === 'general' ? 'General' : tab === 'appearance' ? 'Appearance' : tab === 'shortcuts' ? 'Shortcuts' : tab === 'credentials' ? 'Credentials' : tab === 'my-nodes' ? 'My Nodes' : tab === 'registry' ? 'Registry' : tab === 'agent' ? 'Agent' : tab === 'local-services' ? 'Local Services' : tab === 'users' ? 'Users' : tab === 'registry-review' ? 'Registry Review' : 'System'}
            </h2>
            <button
          ...
          {/* Tab content */}
          <div className="flex-1 overflow-y-auto px-6 py-4">
            {tab === 'general' ? <GeneralTab /> : tab === 'appearance' ? <AppearanceTab /> : tab === 'shortcuts' ? <ShortcutsTab /> : tab === 'credentials' ? <CredentialsTab /> : tab === 'my-nodes' ? <MyNodesTab /> : tab === 'registry' ? <RegistryTab /> : tab === 'agent' ? <AgentTab /> : tab === 'local-services' ? <LocalServicesTab /> : tab === 'users' ? <UsersTab /> : tab === 'registry-review' ? <RegistryReviewTab /> : <SystemTab />}
          </div>
        </div>
      </div>
    </div>
