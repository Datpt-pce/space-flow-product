...
export function SystemTab() {
  ...
  const [agentUpdate, setAgentUpdate] = useState(null);
  const logRef = useRef(null);

  useEffect(() => {
    let disposed = false;
    const refresh = () => fetchSystemStatus()
      .then(res => { if (!disposed) { setAvailable(res.available); setCanUpdateCode(!!res.canUpdateCode); setAgentVersion(res.version || null); setAgentUpdate(res.agentUpdate || null); } })
      .catch(err => { if (!disposed) { setAvailable(false); setUnavailableReason(err.message); } });
    refresh();
  ...
  return (
    <div className="flex flex-col gap-4 py-1 h-full">
      <div>
        <p className="text-xs font-semibold text-[var(--n400,#9ca3af)] uppercase tracking-wider mb-3">{agentUpdate?.signedUpdates ? 'Cập nhật Agent' : canUpdateCode ? 'Code & thư viện' : 'Thư viện'}</p>
        <div className="flex items-center justify-between">
          <span className="text-sm text-[var(--sub,#374151)]">
            {agentUpdate?.signedUpdates ? 'Cập nhật bản đã xác minh khi Agent hoàn tất công việc.' : canUpdateCode ? 'Lấy code mới nhất + thư viện JS/Python (yt-dlp...)' : 'Cập nhật thư viện JS (root/frontend/backend) + Python (yt-dlp...)'}
          </span>
          {available === false ? (
        ...
        {agentUpdate?.signedUpdates && (
          <p role="status" className="text-xs text-[var(--sub,#374151)] mt-2">
            {({ bridge: 'Sẵn sàng nhận bản cập nhật.', healthy: 'Agent đang hoạt động.', downloading: 'Đang tải bản cập nhật.',
              'waiting-for-idle': 'Chờ công việc đang chạy hoàn tất.', restarting: 'Đang khởi động lại Agent.',
              'rolled-back': 'Đã quay lại bản trước để tiếp tục sử dụng.', failed: 'Cập nhật chưa thành công. Bản hiện tại được giữ lại.' })[agentUpdate.status] || 'Đang kiểm tra cập nhật.'}
            {agentUpdate.error && ` ${agentUpdate.error}`}
          </p>
        )}
        {available === false && (
      ...
      {canUpdateCode && autoUpdateCfg && (
        <div>
          ...
          <label className="flex items-center gap-2 text-sm text-[var(--sub,#374151)] cursor-pointer">
            ...
            />
            {agentUpdate?.signedUpdates ? 'Tự động cập nhật khi có bản mới và Agent đã hoàn tất công việc' : 'Tự động lấy code mới + khởi động lại khi có bản mới (không cần bấm "Cập nhật")'}
          </label>
        ...
        <p className={`text-xs ${pythonWarning ? 'text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-2' : 'text-green-600'}`}>
          {restarting
            ...
            : pythonWarning
              ...
              : agentUpdate?.signedUpdates ? 'Đã kiểm tra bản cập nhật. Agent sẽ chuyển bản khi đủ điều kiện.' : 'Hoàn tất — các thư viện đã ở bản mới nhất hoặc đã được cập nhật.'}
        </p>
      )}
      {status === 'error' && (
