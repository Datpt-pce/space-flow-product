...
const PORT_COLORS = {
  ...
  text_in: 'var(--pt-text, #3b82f6)',
  image:   'var(--pt-image, #a855f7)',
  file:    'var(--pt-file, #94a3b8)',
  files:   'var(--pt-array, #14b8a6)',
  items:   'var(--pt-array, #14b8a6)',
  array:   'var(--pt-array, #14b8a6)',
  trigger: 'var(--pt-any, #6366f1)',
};

function getPortColor(handleId) {
  if (!handleId) return 'var(--pt-any, #9ca3af)';
  const key = Object.keys(PORT_COLORS).find(k => handleId.includes(k));
  return key ? PORT_COLORS[key] : 'var(--pt-any, #9ca3af)';
}
