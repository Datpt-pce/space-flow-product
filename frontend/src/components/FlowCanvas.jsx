...
function ZoomControl() {
  ...
  return (
    <div
      className="absolute flex items-center gap-1 bg-[var(--card,white)] border border-[var(--card-border,#e5e7eb)] rounded-xl shadow-sm px-1.5 py-1"
      style={{ bottom: 16, right: 16, zIndex: 10 }}
    >
      <button
        className="w-6 h-6 flex items-center justify-center rounded-lg hover:bg-[var(--n100,#f3f4f6)] text-[var(--sub,#6b7280)] text-sm font-medium transition-colors"
        onClick={() => zoomOut({ duration: 150 })}
        title="Zoom out"
      >−</button>
      <button
        className="min-w-[44px] px-1 h-6 flex items-center justify-center rounded-lg hover:bg-[var(--n100,#f3f4f6)] text-[11px] text-[var(--sub,#4b5563)] font-medium transition-colors tabular-nums"
...
export default function FlowCanvas() {
  ...
  const savedTypes = [...new Set(nodes.map(node => node.type))].sort().join(',');
  const nodeTypes = useMemo(() => buildNodeTypes(Object.values(nodeManifests), nodes), [nodeManifests, savedTypes]);
  const edgeTypes = useMemo(() => ({ custom: CustomEdge }), []);

  const onConnect = useCallback(
    (params) => { useStore.setState({ activeNodeHistoryId: null }); storeAddEdge(addEdge(params, edges).slice(-1)[0]); },
    [edges, storeAddEdge]
  );
    ...
    const handleKeyDown = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && !isTyping) {
        if (e.key.toLowerCase() === 'z') {
          if (e.defaultPrevented || useStore.getState().activeModule !== 'flow') return;
          if (e.target.closest('.fixed,[role="dialog"],[aria-modal="true"]')) return;
          e.preventDefault();
          const state = useStore.getState();
          if (state.activeNodeHistoryId) {
            if (e.shiftKey) void state.redoNode(state.activeNodeHistoryId); else void state.undoNode(state.activeNodeHistoryId);
          } else if (e.shiftKey) state.redo(); else state.undo();
        }
        else if (e.key === 'c') {
          if (window.getSelection()?.toString()) return; // đang bôi đen text — để browser tự copy
          e.preventDefault();
          copySelected();
        }
  ...
  const onDrop = useCallback(async (e) => {
    ...
    // File drop from outside the browser window — resolve real path (no upload), create node(s)
    if (e.dataTransfer.files.length > 0) {
      ...
      if (paths.length === 1) {
        ...
        const fileName = paths[0].replace(/\\/g, '/').split('/').pop();
        addNodeToCanvas(mediaManifest, dropPos, { file_path: paths[0], file_type: getFileType(fileName) });
        return;
      }

      // Thả nhiều file cùng lúc → gộp vào 1 node List thay vì tạo nhiều node Media rời rạc
      const listManifest = nodeManifests['list'];
      if (!listManifest) return;
      const types = paths.map(p => getFileType(p.replace(/\\/g, '/').split('/').pop()));
      const file_type = types.every(t => t === types[0]) ? types[0] : 'any';
      addNodeToCanvas(listManifest, dropPos, { files: paths, file_type });
      return;
    }
  ...
  return (
    <div className="absolute inset-0">
      <ReactFlow
        ...
        onNodeDragStart={() => { useStore.setState({ activeNodeHistoryId: null }); useStore.getState()._pushUndo(); }}
        onConnect={onConnect}
        nodeTypes={nodeTypes}
        edgeTypes={edgeTypes}
        onPaneClick={() => { useStore.setState({ activeNodeHistoryId: null }); selectNode(null); closePalette(); closeContextMenu(); }}
        onContextMenu={onContextMenu}
        onNodeContextMenu={onNodeContextMenu}
        onDrop={onDrop}
        onDragOver={onDragOver}
        fitView
        style={{ background: 'var(--canvas, #f5f5f5)' }}
      ...
      >
        {canvasSettings.backgroundVariant !== 'none' && (
          <Background color="var(--n300, #d1d5db)" gap={24} size={1.5} variant={canvasSettings.backgroundVariant} />
        )}
      </ReactFlow>
      <ZoomControl />
    </div>
