...
export default function ContextMenu() {
  ...
  return (
    <div
      ref={menuRef}
      className="sf-motion-menu fixed z-[9999] bg-[var(--card,#fff)] border border-[var(--card-border,#e5e7eb)] rounded-xl shadow-xl py-1.5 px-1"
    ...
    >
      ...
      {type === 'item' && itemKind === 'row' && (
        // Row bảng của ListNodeBeta — item table row (config.tableItems), xoá theo token giống text.
        <>
          <MenuItem icon={Pencil} label="Edit" onClick={() => { requestEditListItem(targetId, itemToken); closeContextMenu(); }} />
          <Divider />
          <MenuItem icon={Trash2} label="Xoá" onClick={() => { removeListItemBeta(targetId, itemToken); closeContextMenu(); }} danger />
      ...
      {type === 'item' && !itemKind && (
        // Hành vi mặc định cũ cho call site không truyền itemKind — không còn call site nào
        // dùng nhánh này sau khi gộp List All/List Content vào node List (2026-08-22), giữ lại
        // phòng trường hợp có node khác gọi openContextMenu({type:'item'}) không kèm itemKind.
        <>
